...
#ifdef NATIVE_PORT
            /* The VR OPTIONS page owns the screen while it is open: the
             * race HUD stands down so the page reads clean, and returns the
             * moment the page closes. Pause-only, so no gameplay-bearing
             * HUD state can be starved (the Banjo skipped-draw lesson). */
            {
                extern s32 mdkr_vr_menu_open(void);
                if (mdkr_vr_menu_open()) {
                    return;
                }
            }
#endif
