...
#ifdef NATIVE_PORT
#include "vr/vr.h" /* the presentation cull wedge; inline no-ops when flat */
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/*
 * DRAW-SIDE ONLY: the same three planes at the widest wedge the PRESENTATION
 * needs. A headset sees about 100 degrees per eye where the authored object
 * wedge is 130/100 -- a 52.4 degree half-angle -- so a prop leaves the wedge
 * with about two degrees to spare at a dead-ahead head, and the first degree
 * of head turn spends it. That is trees and props vanishing at the edge of
 * vision while the track stays (segments are large boxes admitted on any
 * corner).
 *
 * Never read by scene_authoritative_render_tick or
 * scene_visibility_prepare_viewport: widening those would make "how wide you
 * can see" a simulation input, exactly the line the Draw distance note above
 * draws for "how far" (the tick's set feeds Object_Racer.unk201, which drives
 * AI steering, a balloon RNG roll and particle emission).
 */
static Vec4f sPresentationCullPlanes[3];
static f32 sPresentationCullSideX = 130.0f;
static s32 sPresentationWedgeWidened;
/* Which wedge block_visible() tests against. Defaults to the set the tick
 * has always used; the DRAW's own segment traversal swaps it for the
 * presentation wedge and puts it back. */
static const Vec4f *sSegmentCullPlanes = D_8011D0F8;

/*
 * sideX for a HORIZONTAL FULL angle in degrees. The plane template puts the
 * far edge at |dz| = 100, so sideX = 100 * tan(fov / 2), times the same 5%
 * hysteresis the level-geometry wedge uses.
 *
 * Three inputs have no tangent worth taking and each clamps UP to the cap,
 * never down: a half-angle at or past 87 degrees (a wide head turn -- the
 * lens block above falls back to 130 there, which would make the WIDEST
 * turns cull the HARDEST, backwards), and vr_cull_fov_deg()'s 360 "cull
 * nothing" sentinel, which lands in the same branch. Zero means NO DATA
 * (flat build, or before the head rest pose is taken) and keeps the
 * authored wedge -- it is not "zero degrees".
 */
static f32 mdkr_cull_side_x_for_fov(f32 fovDeg) {
    f32 halfDeg;
    f32 sideX;
    ...
    sideX = tanf(halfDeg * (3.14159265358979323846f / 180.0f)) * 1.05f * 100.0f;
    if (sideX < 130.0f) {
        return 130.0f;
    }
    if (sideX > 2000.0f) {
...
static s32 scene_wide_draw_route(Object *obj, MdkrViewportRoutePass pass,
    ...
    if (obj == NULL ||
        (mdkr_enh_draw_distance_scale() == 1.0f &&
         !sPresentationWedgeWidened)) {
        goto tally;
    }
    if (obj->trans.flags & OBJ_FLAGS_PARTICLE) {
        /* Particles are exempt from the authored distance test entirely
         * (check_if_in_draw_range_impl's OBJ_FLAGS_PARTICLE guard), so the
         * "authored admits, never added here" rule below is VACUOUS for them
         * -- and used to refuse every particle the tick's narrower wedge left
         * unrouted: sparkles and splashes popping at a headset eye's edge
         * where props now survive. Under a widened presentation wedge their
         * admission is re-derived from this pass's own segment table. Route
         * entries still win (nothing draws twice), and only the opaque pass
         * renders particle objects, so the other passes keep the refusal. */
        if (!sPresentationWedgeWidened || routeAdmitted ||
            pass != MDKR_VIEWPORT_ROUTE_OPAQUE ||
            (obj->trans.flags & visibleFlags) ||
            (obj->trans.flags & OBJ_FLAGS_UNK_0080) ||
            !segmentVisible[obj->segmentID + 1]) {
            goto tally;
        }
        *visible = 255;
        *renderOpacity = scene_camera_obstruction_clamp(obj, 255);
        routeAdmitted = TRUE;
        extended = TRUE;
        goto tally;
    }
...
#define CHECK_DRAW_RANGE_RENDER(o) check_if_in_draw_range(o)
    ...
    if (gCurrentLevelModel->numberOfSegments > 1) {
        numberOfSegments = 0;
#ifdef NATIVE_PORT
        /* DRAW-side traversal only: walk the segments the PRESENTATION can
         * see. Restored immediately below, so the tick's own walk
         * (scene_visibility_prepare_viewport) still decides visibility --
         * and therefore the AI's "onscreen" contract -- on the authored
         * wedge. Without this the object wedge above admits props whose
         * segment was never walked, and nothing reaches the screen. */
        sSegmentCullPlanes = sPresentationCullPlanes;
#endif
        traverse_segments_bsp_tree(0, 0, gCurrentLevelModel->numberOfSegments - 1, segmentIds, &numberOfSegments);
#ifdef NATIVE_PORT
        sSegmentCullPlanes = D_8011D0F8;
#endif
    } else {
        numberOfSegments = 1;
        segmentIds[0] = 0;
...
void func_8002A31C(s32 renderPass) {
    ...
    /*
     * The PRESENTATION wedge. vr_cull_fov_deg() is the only value in the
     * build that knows the headset: a horizontal FULL angle in degrees about
     * the game camera's axis, already carrying both eye frusta, the head's
     * free-look offset and the layer's own slop.
     *
     * Gated on vr_is_active(), NOT vr_stereo_active(): the display list is
     * BUILT here, in the flat frame, and vr_stereo_active() is only true
     * between vr_begin_eye and vr_end_eye, which run afterwards on the list
     * this decided. A wedge chosen per eye would arrive a whole build late.
     */
    sPresentationCullSideX = cullSideX;
    sPresentationWedgeWidened = FALSE;
    {
        f32 presentationFov;
        f32 presentationSideX;
        /*
         * MDKR_CULL_FOV supplies the runtime's answer headlessly, the same
         * stand-in shape as MDKR_VR_FAKE_YAW: <deg> forces a wedge, and
         * "off" forces the AUTHORED wedge even under a live session. The
         * off switch is not decoration -- the first A/B of this change had
         * no control, because the fake rig reports vr_is_active() and both
         * arms widened identically.
         */
        static f32 forcedFov = -1.0f;
        static s32 forcedOff = -1;
        if (forcedOff < 0) {
            const char *env = getenv("MDKR_CULL_FOV");
            forcedOff = (env != NULL && env[0] == 'o') ? 1 : 0;
            forcedFov = (env != NULL && env[0] != '\0' && !forcedOff)
                            ? (f32) atof(env) : 0.0f;
        }
        presentationFov = (vr_is_active() && !forcedOff)
                              ? vr_cull_fov_deg() : 0.0f;
        /* The flat view modes move the eye off the camera too, so they need
         * the same widening for the same reason (camera.c). Zero in third
         * person, which leaves the authored wedge exactly as it was. */
        if (!forcedOff && !vr_is_active()) {
            const f32 flatFov = mdkr_flat_cull_fov_deg();
            if (flatFov > presentationFov) {
                presentationFov = flatFov;
            }
        }
        if (!forcedOff && forcedFov > presentationFov) {
            presentationFov = forcedFov;
        }
        presentationSideX = mdkr_cull_side_x_for_fov(presentationFov);
        /*
         * THE WEDGE MUST NOT BREATHE, and this is the fix for props blinking
         * in the adventure hub.
         *
         * vr_cull_fov_deg() is recomputed on every PRESENTED frame from the
         * live head pose, while this list is built at tick rate. Two builds
         * therefore sample two different head poses and ask for two different
         * wedge widths, so anything sitting near the edge was admitted on one
         * build and refused on the next -- a binary on/off with no fade,
         * which is exactly what a flickering prop is. The hub is where it
         * shows worst because that is where you look around and down, and a
         * frustum ray near vertical trips the layer's 360-degree "open
         * everything" answer, a sevenfold jump in width that ordinary
         * tracking noise can flip on and off.
         *
         * So the wedge OPENS instantly -- never lose geometry waiting for a
         * filter -- and CLOSES slowly, a few percent per build. A width that
         * only ever eases inward cannot alternate, and the cost of holding a
         * wider wedge for a second longer is CPU in the BSP walk, never a
         * missing prop.
         */
        {
            static f32 sHeldSideX = 0.0f;
            const f32 kCloseRate = 0.03f;   /* per build; ~2 s from open to shut */
            if (presentationSideX >= sHeldSideX) {
                sHeldSideX = presentationSideX;
            } else {
                sHeldSideX -= (sHeldSideX - presentationSideX) * kCloseRate;
                if (sHeldSideX < presentationSideX) {
                    sHeldSideX = presentationSideX;
                }
            }
            presentationSideX = sHeldSideX;
        }
        if (presentationSideX > sPresentationCullSideX) {
            sPresentationCullSideX = presentationSideX;
            sPresentationWedgeWidened = TRUE;
        }
        {
            /* Say what the wedge actually resolved to, once. A widening that
             * silently does not happen looks exactly like a widening that
             * happened and found nothing. */
            /* On CHANGE, not once: the first plane build happens before the
             * VR layer has a head pose, so a one-shot sample reports the
             * "no data" wedge forever and looks like a dead feature. */
            static f32 wedgeLoggedSide = -1.0f;
            static s32 wedgeLogCount = 0;
            if (wedgeLoggedSide != sPresentationCullSideX &&
                wedgeLogCount < 12 && getenv("MDKR_DRAWDIST_TRACE") != NULL) {
                wedgeLoggedSide = sPresentationCullSideX;
                wedgeLogCount++;
                fprintf(stderr,
                        "[CULLWEDGE] vr=%d fov=%.1f sideX=%.1f (authored %.1f) "
                        "widened=%d\n",
                        vr_is_active() ? 1 : 0, (double)presentationFov,
                        (double)sPresentationCullSideX, (double)cullSideX,
                        (int)sPresentationWedgeWidened);
                fflush(stderr);
            }
        }
    }
    ...
    for (i = 0; i < ARRAY_COUNT(D_8011D0F8); i++) {
        ...
        mdkr_build_cull_plane(cameraMatrix, i, sPresentationCullSideX,
                              &sPresentationCullPlanes[i]);
    }
#else
...
s32 block_visible(LevelModelSegmentBoundingBox *bb) {
    ...
    for (j = 0; j < 3; j++) {
#ifdef NATIVE_PORT
        /* Whichever wedge the CALLER selected. The tick's visibility walk
         * leaves this at D_8011D0F8 (its result is a simulation input); only
         * the draw's own traversal points it at the presentation wedge, and
         * restores it immediately after. Widening the object wedge alone
         * recovered nothing -- measured, extended stayed 0 -- because a
         * newly-admitted prop still needs its SEGMENT to have been walked. */
        dirX = sSegmentCullPlanes[j].x;
        dirY = sSegmentCullPlanes[j].y;
        dirZ = sSegmentCullPlanes[j].z;
        dirW = sSegmentCullPlanes[j].w;
#else
        dirX = D_8011D0F8[j].x;
        dirY = D_8011D0F8[j].y;
        dirZ = D_8011D0F8[j].z;
...
#ifdef NATIVE_PORT
            /*
             * The plane set is the CALLER's choice, and only one caller ever
             * passes anything but sFaithfulCullPlanes: the read-only draw
             * extender, which may pass the presentation wedge so a headset's
             * wider field is not starved. Everything that writes back -- the
             * fixed-step visibility prepass and its authored AI "onscreen"
             * contract -- keeps the original guard-band frustum.
             */
            x = cullPlanes[i].x;
            z = cullPlanes[i].z;
            w = cullPlanes[i].w;
...
static s32 scene_object_wide_draw_range(Object *obj, s32 *outOpacity,
    ...
    *outAuthored = check_if_in_draw_range_impl(obj, outAuthoredOpacity, 1.0f,
                                               sFaithfulCullPlanes);
    *outOpacity = obj->opacity;
    /* The ONE widened read in the build: the player's draw distance AND the
     * presentation wedge, both draw-side, neither written back. */
    return check_if_in_draw_range_impl(obj, outOpacity,
                                       mdkr_enh_draw_distance_scale(),
                                       sPresentationCullPlanes);
}
#endif
