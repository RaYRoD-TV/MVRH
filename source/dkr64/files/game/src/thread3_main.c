...
#ifdef NATIVE_PORT
/*
 * VR: stereo eyes render only while the 3D WORLD is what the player should
 * be inside. Frontend menus are 2D compositions -- drawn flat into two
 * asymmetric eye frusta they cannot fuse ("crosseyed menus") -- so those
 * read on the theater panel instead. One game-side answer, so the platform
 * never guesses at menu state.
 *
 * The IN-RACE PAUSE stays live on purpose: the world keeps rendering in
 * stereo (frozen), the pause dialogs are 2D and ride the HUD/menu quad,
 * and every VR OPTIONS knob is therefore visible in the world AS IT MOVES
 * -- world size, stereo, seat, all of it, no unpause round-trip. The
 * companion answer below tells the platform the menu quad is up.
 */
s32 mdkr_vr_world_is_live(void) {
    return gGameMode == GAMEMODE_INGAME;
}

/* VR: is the in-race pause menu up? Drives the HUD quad's MENU anchoring
 * (fixed readable width at the menu distance) while stereo stays live.
 *
 * The FINISH-LINE flow counts: it forces gIsPaused FALSE every frame
 * (above, ~line 625), but its results board and TRY AGAIN options are a
 * menu in every sense a headset cares about -- 2D, needing the readable
 * menu quad and the dim, over a world that stays live in stereo behind
 * them. Without this clause they rode the race-HUD anchoring, tiny and
 * undimmed: the broken finish menu. */
s32 mdkr_vr_menu_is_up(void) {
    return gGameMode == GAMEMODE_INGAME &&
           (is_game_paused() || gPostRaceViewPort);
}
#endif
