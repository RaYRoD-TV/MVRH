...
#ifdef NATIVE_PORT
    /* One-shot: the racer hitbox radii in game units, for deriving the
     * world's units-per-metre (a real kart is roughly 1.5 m long). */
    {
        static int sRadiiLogged = 0;
        if (!sRadiiLogged && mdkr_trace_enabled()) {
            sRadiiLogged = 1;
            mdkr_trace("racer hitbox radii: car=%.2f hover=%.2f plane=%.2f "
                       "[3]=%.2f [4]=%.2f units",
                       racerSize[0], racerSize[1], racerSize[2],
                       racerSize[3], racerSize[4]);
        }
    }
#endif
