...
/* THE VIEW MODES ON A MONITOR. Third person is the authored camera; first
 * person and diorama move the DRAW's eye only (never the tick's, so the race
 * is identical whichever view you play in). See camera.c's block comment. */
s32 mdkr_flat_view_mode(void);
s32 mdkr_flat_first_person_active(void);
/* How far the draw's cull wedge has to open for that eye. 0 = authored. */
