...
ObjectHeader *load_object_header(s32 index) {
    ...
    if (address != NULL) {
         ...
         * are read and patched into arena pointer values. */
...
void render_object(Gfx **dList, Mtx **mtx, Vertex **verts, Object *obj) {
    ...
    /* Sitting in the driver's seat on a monitor: your own kart surrounds the
     * camera, so drawing it fills the screen with its own bodywork. The
     * headset hides it at the eye walk; this is the flat half. DRAW-only --
     * the kart still exists, still moves, still collides, and its shadow is
     * drawn by the caller, so nothing about the race changes because you
     * cannot see your own machine. */
    {
        extern s32 mdkr_flat_first_person_active(void);
        if (mdkr_flat_first_person_active() &&
            obj == get_racer_object_by_port(mdkr_net_roster_runtime_viewport_player(0u, PLAYER_ONE))) {
            return;
        }
    }
#endif
 ...
 *   1-2. `collisionPlanes` and `collisionFacets` are `dkrptr32` values here, so
