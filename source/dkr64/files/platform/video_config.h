...
MdkrVideoRuntimeResult mdkr_video_config_runtime_set(MdkrVideoKey key,
                                                     const char *value);
/* VR: raise a LIVE setting's value for this process only (never persisted);
 * yields to an env/CLI choice or an equal-or-higher current value. */
void mdkr_video_config_vr_floor(MdkrVideoKey key, float value);
/* VR: set a LIVE setting in either direction, process-local, never
 * persisted; yields to an env/CLI choice. For values derived from the
 * device (the eye render scale), which must be able to fall as well. */
void mdkr_video_config_vr_set_live(MdkrVideoKey key, float value);
/* VR: the same live-only, env/CLI-yielding set for TEXT (enum) keys --
 * FrameLimit "display", MotionSmoothing "interpolate". */
