 ...
 * caller passes &gfx_opengl_api (or the active backend). Calls rapi->init().
 */
...
/** Retire prior-stage pointer values after its final graphics task is drained. */
...
/* VR HUD pass: re-walk the retained list drawing ONLY the 2D overlay
 * candidates, into the backend's transparent HUD texture. Returns false
 * without ENABLE_VR or when the walk is refused. */
bool gfx_dkr_replay_walk_vr_hud(void);

/* Running count of billboard vertices that took the vertex-path counter-roll
 * (the unregistered-sprite fallback). Diagnostic surface for the eye log. */
uint64_t gfx_dkr_eye_billboard_rolled_count(void);
/* Running count of billboard vertices the shipped card projection rebuilt as
 * world quads through the eye lens (the default sprite path in stereo). */
uint64_t gfx_dkr_eye_card_vertex_count(void);

/* Screen roll (radians) of the eye view the next eye walk renders; F3DDKR
 * billboard sprite bases are clip-space and must turn with the world under
 * head roll. Set per eye by the VR eye pass; keep 0 otherwise. */
void gfx_dkr_set_eye_billboard_roll(float rad);
/* Both card axes for the unregistered-sprite fallback:
 * { right.xyz, up.xyz (eye-view space), P00, P11, P20, P21 }. The walk
 * re-projects authored screen offsets along these at each sprite's own
 * anchor, so those cards behave exactly like the registered world-plane
 * path (planted; thin and lean by true perspective; never re-face the eye
 * as the head moves). NULL resets to identity (no turn). */
void gfx_dkr_set_eye_billboard_axes(const float axes[16]);

/* First person: the player racer's transform address. Eye walks skip every
 * primitive whose active matrix registered under this presentation owner
 * (kart, rider sprite, wheels -- one address covers them all). NULL shows
 * the kart. The flat mirror is untouched (eye-bracket-gated). */
