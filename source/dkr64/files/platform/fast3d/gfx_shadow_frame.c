...
bool gfx_shadow_matrix_register(
    ...
    /* MDKR_VR_MATRIXPROBE: one accepted sample key per tick, to compare with
     * the addresses the stereo replay looks up. Temporary diagnosis aid. */
    {
        static int probe = -1;
        if (probe < 0) {
            probe = getenv("MDKR_VR_MATRIXPROBE") != NULL;
        }
        if (probe) {
            fprintf(stderr, "[vrprobe] register key=%p site=%d ordinal=%zu\n",
                    key, site, s_matrix_count);
        }
    }
...
size_t gfx_shadow_matrix_live_count(void) {
    return s_matrix_count;
}
...
bool gfx_shadow_replay_restore(
    ...
    for (size_t index = 0; index < s_matrix_count; index++) {
         ...
         *
         * The skip's real precondition is byte equality, so test THAT and not
         * the alpha alone: a camera SUBSTITUTION at the authored instant (the
         * VR eye pass) also arrives with numerator 0, and keying the skip on
         * the alpha silently discarded its override for every matrix --
         * measured as matrix hits=0 and two identical eyes. */
        if (overrides[entry->viewport].numerator == 0u &&
            memcmp(entry->binding.captured_view_projection,
                   overrides[entry->viewport].view_projection,
                   sizeof(entry->binding.captured_view_projection)) == 0) {
            continue;
        }
