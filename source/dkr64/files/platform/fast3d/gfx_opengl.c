...
/* MGB64_PORTMASTER_GLES is what the Android/Quest target builds with. This file
 ...
 * PortMaster/GLES handheld target with the same need. The branches were carried
 * for years only as a clean diff against mgb64's copy and are live code here
 * now: they are the arm a Quest actually renders through, so a change to one of
 * them is a change to the shipped headset build. */
...
#include "../vr/vr.h"          /* the eye pass owns the render target */
#include "../gfx_pc.h"
#include "front.h"
...
struct ShaderProgram {
    ...
    GLint vr_hud_alpha_floor_location;
...
#ifdef ENABLE_VR
/* True while the VR HUD walk renders; that pass owns alpha state (see
 * gfx_opengl_set_blend_mode and the uVrHudAlphaFloor uniform) so its texture
 * carries real coverage. */
static bool g_vr_hud_pass;
static GLuint g_vr_hud_fbo;
static GLuint g_vr_hud_tex;
static int g_vr_hud_w, g_vr_hud_h;
#endif
/* Defined (with initializer) beside the other blend-mode flags below;
 * named here because set_uniforms keys the HUD alpha floor on it. */
static bool g_blend_disabled;
...
static void gfx_opengl_set_uniforms(struct ShaderProgram *prg) {
    ...
    if (prg->vr_hud_alpha_floor_location >= 0) {
#ifdef ENABLE_VR
        /* Floor alpha to 1 ONLY for non-blended HUD draws, where the
         * combiner's alpha-0 output relied on RDP write semantics and alpha
         * means coverage. On BLENDED draws fragment alpha IS the blend
         * factor: flooring it turned every translucent 2D fully opaque, and
         * the trailing full-screen fade quad then blanketed the whole HUD
         * texture with black -- measured by the HUDEND census as 4096/4096
         * alpha, 0/4096 rgb. Blended draws keep their real alpha; the
         * ONE / ONE_MINUS_SRC_ALPHA alpha blend accumulates their coverage. */
        glUniform1f(prg->vr_hud_alpha_floor_location,
                    (g_vr_hud_pass && g_blend_disabled) ? 1.0f : 0.0f);
#else
        glUniform1f(prg->vr_hud_alpha_floor_location, 0.0f);
#endif
    }
...
static struct ShaderProgram *gfx_opengl_create_and_load_new_shader(uint64_t shader_id0, uint32_t shader_id1) {
    ...
    append_line(fs_buf, sizeof(fs_buf), &fs_len, "out vec4 fragColor;");
#endif
    /* VR HUD pass: floors written alpha to 1 so the HUD texture carries
     * coverage (DKR HUD combiners output alpha 0 with blending off, relying
     * on RDP write semantics). 0.0 everywhere else -- a plain max, so the
     * ordinary paths are bit-identical. */
    append_line(fs_buf, sizeof(fs_buf), &fs_len,
                "uniform float uVrHudAlphaFloor;");
    if (cc_features.opt_world_pos) {
    ...
    append_line(fs_buf, sizeof(fs_buf), &fs_len,
                "fragColor.a = max(fragColor.a, uVrHudAlphaFloor);");
    append_line(fs_buf, sizeof(fs_buf), &fs_len, "}");
    ...
    prg->n64_filter_scale_location = prg->used_n64_filter
        ...
        : -1;
    prg->vr_hud_alpha_floor_location =
        glGetUniformLocation(shader_program, "uVrHudAlphaFloor");
...
/* MDKR_VR_RASTERPROBE: temporary stereo diagnosis. Tags every viewport and
 * scissor the interpreter issues with which pass it landed in, so an eye pass
 * rendering through the wrong rectangle names itself. */
static int gfx_opengl_raster_probe(void) {
    static int enabled = -1;
    if (enabled < 0) {
        enabled = getenv("MDKR_VR_RASTERPROBE") != NULL;
    }
    return enabled;
}
...
#ifdef ENABLE_VR
    if (gfx_opengl_raster_probe() && vr_stereo_active()) {
        fprintf(stderr, "[rasterprobe] EYE viewport %d,%d %dx%d\n",
                x, y, width, height);
    }
#endif
...
#ifdef ENABLE_VR
    if (gfx_opengl_raster_probe() && vr_stereo_active()) {
        fprintf(stderr, "[rasterprobe] EYE scissor  %d,%d %dx%d\n",
                x, y, width, height);
    }
#endif
...
#ifdef ENABLE_VR
    if (g_vr_hud_pass) {
        static int noBlendFix = -1;
        if (noBlendFix < 0) {
            noBlendFix = getenv("MDKR_VR_HUD_NOBLENDFIX") != NULL;
        }
        /* The VR HUD texture needs real COVERAGE in its alpha channel -- the
         * quad layer composites with it. The ordinary paths either mask
         * alpha writes or blend it with back-buffer semantics, which leaves
         * the texture fully transparent (measured: perfect HUD in RGB, alpha
         * 0 everywhere). Own the state: alpha always writable, and blended
         * draws accumulate coverage. */
        glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
        if (!noBlendFix &&
            !(mode == GFX_BLEND_DISABLED ||
              mode == GFX_BLEND_ALPHA_RDP_MEMORY ||
              mode == GFX_BLEND_ALPHA_RDP_CVG_MEMORY)) {
            if (mode == GFX_BLEND_MODULATE) {
...
#ifdef ENABLE_VR
    if (g_vr_hud_pass) {
        /* MDKR_VR_RASTERPROBE: name the actual GL state each HUD-pass batch
         * drew with -- the diagnosis that ends the "31 draws issued, zero
         * pixels landed" class. Blend FUNCS, depth, cull, A2C, textures, the
         * live alpha-floor uniform and a raw vertex ride along: the first
         * probe logged only the state that turned out to be innocent. */
        static int logged = 0;
        if (gfx_opengl_raster_probe() && logged < 48) {
            GLint fbo = 0, vp[4] = {0}, sc[4] = {0}, prog = 0;
            GLint bsrc = 0, bdst = 0, bsrca = 0, bdsta = 0;
            GLint tex0 = 0;
            GLboolean cm[4] = {0}, blend, scissor, depth, dmask, cull, a2c;
            GLfloat floorv = -1.0f;
            logged++;
            ...
            a2c = glIsEnabled(GL_SAMPLE_ALPHA_TO_COVERAGE);
            if (current_shader_program != NULL &&
                current_shader_program->vr_hud_alpha_floor_location >= 0) {
                glGetUniformfv(
                    (GLuint)prog,
                    current_shader_program->vr_hud_alpha_floor_location,
                    &floorv);
            }
            fprintf(stderr,
                    "[rasterprobe] HUDDRAW tris=%zu fbo=%d (hud=%u) vp=%d,%d "
                    "%dx%d sc=%d,%d %dx%d scEn=%d blend=%d func=%04x/%04x "
                    "a=%04x/%04x mask=%d%d%d%d depth=%d dmask=%d cull=%d "
                    "a2c=%d prog=%d tex=%d floor=%.2f "
                    "v0=[%.1f %.1f %.1f %.2f]\n",
                    buf_vbo_num_tris, fbo, g_vr_hud_fbo, vp[0], vp[1], vp[2],
                    vp[3], sc[0], sc[1], sc[2], sc[3], (int)scissor,
                    (int)blend, bsrc, bdst, bsrca, bdsta,
                    (int)cm[0], (int)cm[1], (int)cm[2], (int)cm[3],
                    (int)depth, (int)dmask, (int)cull, (int)a2c, prog, tex0,
                    (double)floorv,
                    buf_vbo_len >= 4 ? (double)buf_vbo[0] : 0.0,
                    buf_vbo_len >= 4 ? (double)buf_vbo[1] : 0.0,
                    buf_vbo_len >= 4 ? (double)buf_vbo[2] : 0.0,
                    buf_vbo_len >= 4 ? (double)buf_vbo[3] : 0.0);
        }
    }
#endif
}
...
static bool gfx_opengl_scene_target_enabled(void) {
    ...
    return g_pcRemasterFX ||
           render_scale > 1.001f ||
           /* Under a headset the flat walk's scene is resolved into the
            * window through gfx_opengl_resolve_scene_target, which is also
            * where the eye mirror lands. A window big enough to round the
            * VR render scale down to 1.0 must not silently lose that seam. */
           vr_is_active() ||
...
#ifdef ENABLE_VR
    /*
     * A VR eye pass owns the render target.
     *
     * The VR layer binds its eye framebuffer and then re-walks the finished
     * display list, which enters here through start_frame. Taking either
     * branch below would rebind the target -- to 0 in the ordinary case --
     * and the eye's pixels would land in the desktop back buffer while both
     * eye images stayed at their clear colour. That is exactly the black-eye
     * defect this branch exists to prevent, and it looked like broken stereo
     * matrices rather than a target binding.
     *
     * The eye target is sized to this frame (vr_set_eye_raster_override), so
     * the display list's own viewport commands fill it with no scaling, and
     * the scene/MSAA target is deliberately skipped: the VR path does its own
     * multisampling and resolve.
     */
    if (vr_stereo_active()) {
        glBindFramebuffer(GL_FRAMEBUFFER, (GLuint)vr_eye_target_fbo());
        glViewport(0, 0, vr_eye_raster_width(), vr_eye_raster_height());
    } else
#endif
...
#ifdef ENABLE_VR
    /*
     * Under a headset the window shows the EYE, not the flat chase camera:
     * the last rendered eye is blitted into the window here, where the flat
     * scene would have landed, so the game's 2D still composes over it
     * below. Only while the world is live in stereo (menus and loading keep
     * the flat frame, which is also what the theater panel reads) -- and the
     * world-live answer is the game's own, known before this frame's eye
     * pass runs. The flat scene target still rendered this frame goes
     * unread; its depth no longer describes the picture, so the depth-based
     * output effects stand down for the frame. The HUD rides on top only
     * through the output overlay (native-res UI, the default); with
     * MDKR_UI_NATIVE_RES=0 the 2D lands in the unread scene FBO and the
     * mirror shows the bare eye.
     */
    {
        extern int mdkr_vr_world_is_live(void);
        if (vr_frame_has_eyes() && mdkr_vr_world_is_live() &&
            vr_blit_mirror(drawable_w, drawable_h)) {
            glBindFramebuffer(GL_FRAMEBUFFER, 0);
...
#ifdef ENABLE_VR
/*
 * VR HUD target: a transparent render-resolution texture the HUD-only walk
 * (gfx_dkr_replay_walk_vr_hud) draws into. The VR layer blits it onto the
 * HUD quad, so the 2D reads at a fixed depth in front of the player instead
 * of flat in each eye (which cannot fuse and rides every head motion).
 */
void gfx_opengl_vr_hud_end(void) {
    /* Pass-end census under the raster probe: whether any pixel LANDED in
     * the HUD texture, read before anything later in the frame could touch
     * it -- the bisector between "fragments never landed" and "content was
     * lost between the pass and the dump". */
    if (gfx_opengl_raster_probe() && g_vr_hud_fbo != 0) {
        static int censusLogged = 0;
        if (censusLogged < 8) {
            unsigned char px[64 * 64 * 4];
            int step_x = g_vr_hud_w > 64 ? g_vr_hud_w / 64 : 1;
            int step_y = g_vr_hud_h > 64 ? g_vr_hud_h / 64 : 1;
            int nonzero = 0, alpha_nonzero = 0, samples = 0;
            censusLogged++;
            glBindFramebuffer(GL_FRAMEBUFFER, g_vr_hud_fbo);
            for (int y = 0; y < g_vr_hud_h && samples < 64 * 64;
                 y += step_y) {
                for (int x = 0; x < g_vr_hud_w && samples < 64 * 64;
                     x += step_x) {
                    unsigned char *p = px + samples * 4;
            ...
            fprintf(stderr,
                    "[rasterprobe] HUDEND census: %d/%d rgb nonzero, %d/%d "
                    "alpha nonzero (%dx%d tex)\n",
                    nonzero, samples, alpha_nonzero, samples, g_vr_hud_w,
                    g_vr_hud_h);
        }
    }
    g_vr_hud_pass = false;
    /* Resync the live program so the alpha floor never leaks into the next
     * real frame (A2C reads fragment alpha). */
    if (current_shader_program != NULL) {
        gfx_opengl_set_uniforms(current_shader_program);
    }
}

bool gfx_opengl_vr_hud_begin(void) {
    const int w = (int)gfx_current_dimensions.width;
    const int h = (int)gfx_current_dimensions.height;
    if (w <= 0 || h <= 0) {
        return false;
    }
    if (g_vr_hud_fbo == 0 || w != g_vr_hud_w || h != g_vr_hud_h) {
        if (g_vr_hud_fbo == 0) {
            glGenFramebuffers(1, &g_vr_hud_fbo);
            glGenTextures(1, &g_vr_hud_tex);
        }
        glBindTexture(GL_TEXTURE_2D, g_vr_hud_tex);
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                               GL_TEXTURE_2D, g_vr_hud_tex, 0);
        if (glCheckFramebufferStatus(GL_FRAMEBUFFER) !=
            GL_FRAMEBUFFER_COMPLETE) {
            glDeleteFramebuffers(1, &g_vr_hud_fbo);
            glDeleteTextures(1, &g_vr_hud_tex);
            g_vr_hud_fbo = 0;
            g_vr_hud_tex = 0;
            return false;
        }
    ...
    glClear(GL_COLOR_BUFFER_BIT);
    g_vr_hud_pass = true;
    /* The program bound before this pass keeps its uniform values unless it
     * is switched away and back; resync so the alpha floor is live from the
     * first batch. */
    if (current_shader_program != NULL) {
        gfx_opengl_set_uniforms(current_shader_program);
    }
...
unsigned int gfx_opengl_vr_hud_texture(int *w, int *h) {
    if (w != NULL) {
        *w = g_vr_hud_w;
    }
    if (h != NULL) {
    ...
    return (unsigned int)g_vr_hud_tex;
}

/* Headless proof seam: write the HUD texture as BGRA->TGA. */
bool gfx_opengl_vr_hud_dump(const char *path) {
    FILE *f;
    unsigned char *pixels;
    unsigned char header[18];
    if (g_vr_hud_fbo == 0 || g_vr_hud_w <= 0 || g_vr_hud_h <= 0) {
        return false;
    }
    pixels = (unsigned char *)malloc((size_t)g_vr_hud_w * g_vr_hud_h * 4);
    if (pixels == NULL) {
        return false;
    }
    glBindFramebuffer(GL_FRAMEBUFFER, g_vr_hud_fbo);
#ifdef MGB64_PORTMASTER_GLES
    /* GLES has no GL_BGRA for a read: the only format every ES 3 driver must
     * accept from glReadPixels is GL_RGBA/GL_UNSIGNED_BYTE. Read that and swap
     * the two ends of each pixel, which costs one pass over a dump that only
     * happens when the proof seam is armed and keeps the TGA on disk
     * byte-identical to the desktop one. */
    glReadPixels(0, 0, g_vr_hud_w, g_vr_hud_h, GL_RGBA, GL_UNSIGNED_BYTE,
                 pixels);
    {
        size_t i, count = (size_t)g_vr_hud_w * (size_t)g_vr_hud_h;
        for (i = 0u; i < count; i++) {
            unsigned char red = pixels[i * 4u];
            pixels[i * 4u] = pixels[i * 4u + 2u];
            pixels[i * 4u + 2u] = red;
        }
    }
#else
    glReadPixels(0, 0, g_vr_hud_w, g_vr_hud_h, GL_BGRA, GL_UNSIGNED_BYTE,
                 pixels);
#endif
    f = fopen(path, "wb");
    if (f == NULL) {
    ...
    header[2] = 2;
    header[12] = (unsigned char)(g_vr_hud_w & 0xff);
    header[13] = (unsigned char)((g_vr_hud_w >> 8) & 0xff);
    header[14] = (unsigned char)(g_vr_hud_h & 0xff);
    header[15] = (unsigned char)((g_vr_hud_h >> 8) & 0xff);
    header[16] = 32;
    header[17] = 0x08; /* bottom-left origin (glReadPixels order), 8 alpha bits */
    fwrite(header, 1, sizeof(header), f);
    fwrite(pixels, 1, (size_t)g_vr_hud_w * g_vr_hud_h * 4, f);
    fclose(f);
    free(pixels);
    return true;
}
