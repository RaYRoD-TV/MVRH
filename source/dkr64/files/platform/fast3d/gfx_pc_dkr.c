...
#include "../vr/vr.h"      /* vr_stereo_active: the eye replay suppresses the
                              output overlay (inline no-op without ENABLE_VR) */
...
static int dkr_vr_matrix_probe_enabled(void);
...
static struct {
    ...
    /* Raw SETCIMG/SETZIMG values (the DL w1 values, before dkr_resolve). The
     * z-clear detection compares THESE, not the resolved pointers: on wasm32 the
     * framebuffer/z-buffer segment values (0x01000000 / 0x02000000) can resolve
     ...
     * fill (the M3b symptom). The raw values are width-independent. */
 ...
 * gfx_resolve_addr() for segment values (gSPSegment / G_MW_SEGMENT sets, whose
...
static inline void *dkr_resolve(uint32_t addr) {
     ...
     *     registry first resolves those globals correctly; arena values (vertex/
     ...
     *   - Genuine N64 segment values (framebuffer/zbuffer 0x0N000000) are neither
     ...
     * K+1. Decode task K's values against its retained original window and
     ...
     * (dkr_arena_init), and N64 segment values are 0x01000000..0x0FFFFFFF. So:
...
static bool dkr_shadow_lookup_live(
    ...
    if (!gfx_shadow_matrix_lookup(identity, out)) {
        if (dkr_vr_matrix_probe_enabled() && dkr_replay_pass) {
            fprintf(stderr,
                    "[vrprobe] frame=%d replay MISS: ma=%p identity=%p "
                    "live=%zu frozen=%zu\n",
                    dkr_frame_index, ma, (const void *)identity,
                    gfx_shadow_matrix_live_count(),
                    gfx_shadow_replay_frozen_matrix_count());
        }
        return false;
    }
    if (dkr_vr_matrix_probe_enabled() && dkr_replay_pass) {
        fprintf(stderr, "[vrprobe] frame=%d replay hit: identity=%p\n",
                dkr_frame_index, (const void *)identity);
    }
    ...
    if (!out->key_bytes_valid || ma == NULL ||
        ...
        if (dkr_vr_matrix_probe_enabled() && dkr_replay_pass) {
            fprintf(stderr,
                    "[vrprobe] frame=%d tenancy GUARD: identity=%p "
                    "key_bytes_valid=%d ma=%p room=%zu\n",
                    dkr_frame_index, (const void *)identity,
                    (int)out->key_bytes_valid, ma,
                    ma != NULL ? dkr_arena_room(ma) : (size_t)0);
        }
    ...
    if (memcmp(dkr_replay_pass && out->walked_key_bytes_valid
        ...
        if (dkr_vr_matrix_probe_enabled() && dkr_replay_pass) {
            const uint32_t *expect =
                (const uint32_t *)(dkr_replay_pass && out->walked_key_bytes_valid
                                       ? out->walked_key_bytes
                                       : out->key_bytes);
            const uint32_t *seen = (const uint32_t *)ma;
            int first = -1;
            for (int w = 0; w < 16; w++) {
                if (expect[w] != seen[w]) { first = w; break; }
            }
            fprintf(stderr,
                    "[vrprobe] frame=%d tenancy BYTES: identity=%p "
                    "walked_valid=%d firstdiff=w%d expect=%08x seen=%08x\n",
                    dkr_frame_index, (const void *)identity,
                    (int)out->walked_key_bytes_valid, first,
                    first >= 0 ? expect[first] : 0u,
                    first >= 0 ? seen[first] : 0u);
        }
        /*
...
/*
 * The VR eye pass renders the WORLD only. 2D drawn flat into both eyes sits
 * at a different angular position in each asymmetric frustum -- unfusable
 * ("crosseyed" HUD/menus) -- and, being locked to the eye frame, it rides
 * every head motion, which reads as the world tilting when the head tilts.
 * The overlay-candidate classification is the same one the flat frame's
 * output overlay uses, so the split matches what the game itself calls HUD.
 *
 * The HUD comes back on its OWN pass: one more walk of the same retained
 * list (dkr_replay_hud_only) that draws ONLY the 2D candidates, into a
 * transparent texture the VR layer parks on the HUD quad in front of the
 * player.
 */
static bool dkr_replay_hud_only = false;

/* Screen roll of the current eye view (radians). F3DDKR billboards add their
 * sprite-local offsets in CLIP space, so a sprite's axes are literally
 * screen axes: under head roll the flora and particles stay screen-upright
 * while the world counter-rolls -- measured as billboards riding the head.
 * The eye walk rotates the sprite basis by this angle so billboards turn
 * with the world. Zero outside the eye walks. */
static float dkr_eye_billboard_roll = 0.0f;
static uint64_t dkr_eye_billboard_rolled;
static uint64_t dkr_hud_walk_drawn, dkr_hud_walk_skipped;

static bool  dkr_eye_bb_turn = false;
/* BOTH card axes in eye-view space plus the projection scalars, for the
 * per-anchor re-projection of authored screen offsets:
 *   ox = (cx/P00)*(P00*r.x + r.z*(P20+nx)) + (cy/P11)*(P00*u.x + u.z*(P20+nx))
 *   oy = (cx/P00)*(P11*r.y + r.z*(P21+ny)) + (cy/P11)*(P11*u.y + u.z*(P21+ny))
 * -- the exact world-plane behavior evaluated at each sprite's own anchor
 * NDC (nx, ny): planted cards that thin and lean by true perspective
 * instead of re-facing the eye as the head moves, and IDENTITY when the
 * axes are at rest (r = e0, u = e1, z = 0), so a level head renders
 * bit-identically. Subsumes the old single counter-roll (a pure roll makes
 * these formulas the pixel-space rotation the old c/s applied). */
static float dkr_eye_bb_rx = 1.0f;
static float dkr_eye_bb_ry = 0.0f;
static float dkr_eye_bb_rz = 0.0f;
static float dkr_eye_bb_ux = 0.0f;
static float dkr_eye_bb_uy = 1.0f;
static float dkr_eye_bb_uz = 0.0f;
static float dkr_eye_bb_p00 = 1.0f;
static float dkr_eye_bb_p11 = 1.0f;
/* The flat authoring lens (see gfx_dkr_set_eye_billboard_axes). Zero means
 * "not published", never "identity" -- an identity default would silently
 * reinstate the very oversize this pair exists to remove. */
static float dkr_eye_bb_flat_p00 = 0.0f;
static float dkr_eye_bb_flat_p11 = 0.0f;

/*
 * BOTH VR BILLBOARD REBUILDS ARE OFF BY DEFAULT. What ships instead is the
 * per-vertex card projection in dkr_sp_vertex (search "THE SHIPPED CARD
 * PROJECTION"): the authored card, sized by the lens that authored it and
 * drawn as a world quad through the eye lens with every vertex carrying its
 * own w. The rest of this note is the history of the two rebuilds it
 * replaced, kept because the failure it describes is the one the shipped
 * path is built not to repeat.
 *
 * The rebuilds were switched off, and sprites drawn exactly as the game
 * authored them, on 2026-08-16:
 *
 * They were built to make cards face each EYE rather than the flat camera.
 * Measured in the headset, that is what they actually did: turning both off
 * removed the tearing of world geometry outright, and stopped the palms
 * stretching -- after three separate corrections inside those paths had
 * changed nothing. It also removes a defect no correction would have
 * reached, because it was the FEATURE: a card that re-faces per eye tilts
 * and swims as the head moves, and a tree has no business leaning with you.
 *
 * The authored card is a flat quad facing the game's camera. In stereo that
 * is not geometrically perfect -- it is a compromise the N64 made and this
 * port now keeps -- but it is stable, it is what the game's art was drawn
 * for, and stable beats clever in something strapped to your face.
 *
 * MDKR_VR_BB=on / MDKR_VR_BBWORLD=on put the rebuilds back for anyone who
 * wants to work on them. They are kept, not deleted: the geometry in them is
 * sound and the defects found along the way (the authoring-lens divide, the
 * near-eye anchor blowup) are fixed in place, so whoever resumes starts from
 * a correct implementation of an idea that has to earn its way back.
 */
static bool dkr_eye_bb_enabled_env(const char *name) {
    const char *e = getenv(name);
    return e != NULL && (e[0] == '1' || e[0] == 'y' || e[0] == 'Y' ||
                         e[0] == 't' || e[0] == 'T' ||
                         ((e[0] == 'o' || e[0] == 'O') &&
                          (e[1] == 'n' || e[1] == 'N')));
}

/* The world-plane substituted population. Off unless explicitly asked for. */
static bool dkr_eye_bbworld_disabled(void) {
    static int off = -1;
    if (off < 0) {
        off = dkr_eye_bb_enabled_env("MDKR_VR_BBWORLD") ? 0 : 1;
        fprintf(stderr, "[vr] world-plane billboard substitution: %s\n",
                off ? "off (authored cards)" : "ON (opt-in)");
    }
    return off != 0;
}

/* Read once. Must not cost a getenv per vertex. */
static bool dkr_eye_bb_disabled(void) {
    static int off = -1;
    if (off < 0) {
        off = dkr_eye_bb_enabled_env("MDKR_VR_BB") ? 0 : 1;
        fprintf(stderr, "[vr] eye billboard rebuild: %s\n",
                off ? "off (authored cards)" : "ON (opt-in)");
    }
    return off != 0;
}
static float dkr_eye_bb_p20 = 0.0f;
static float dkr_eye_bb_p21 = 0.0f;
/* The eye lens's depth row (P22), so a card vertex's z stays consistent
 * with the w the shipped card projection gives it. */
static float dkr_eye_bb_p22 = 0.0f;
/* Census for the eye log: billboard vertices that took the shipped card
 * projection this session. Zero with stereo live means the path is dark. */
static uint64_t dkr_eye_card_vertices;

/* MDKR_VR_BBCARD=off stands the shipped card projection down (raw authored
 * cards: screen-glued and oversize, but tear-free -- the 2026-08-16 18:05
 * state judged in the headset). Read once. */
static bool dkr_eye_card_enabled(void) {
    static int on = -1;
    if (on < 0) {
        const char *e = getenv("MDKR_VR_BBCARD");
        on = (e != NULL && (e[0] == '0' || e[0] == 'o' || e[0] == 'O' ||
                            e[0] == 'n' || e[0] == 'N' || e[0] == 'f' ||
                            e[0] == 'F')) ? 0 : 1;
        fprintf(stderr, "[vr] sprite card projection: %s\n",
                on ? "on (world quads through the eye lens)"
                   : "OFF (raw authored cards, MDKR_VR_BBCARD)");
    }
    return on != 0;
}
/* The camera RIG in eye-view space: the cylindrical facing origin (never
 * the per-frame head, which made cards swivel with a lean). */
static float dkr_eye_bb_cx = 0.0f;
static float dkr_eye_bb_cy = 0.0f;
static float dkr_eye_bb_cz = 0.0f;

/*
 * Per matrix SLOT: did the eye walk substitute a WORLD-plane sprite basis
 * here? Such a matrix is already world-true and must never take the
 * counter-roll below, or it double-rotates. Everything else in slot 2 that
 * reaches a billboard draw is an authored screen-space basis and does.
 */
static bool dkr_slot_world_billboard[3];

void gfx_dkr_set_eye_billboard_roll(float rad) {
    dkr_eye_billboard_roll = rad;
}

void gfx_dkr_set_eye_billboard_axes(const float axes[16]) {
    dkr_eye_bb_turn = false;
    dkr_eye_bb_rx = 1.0f; dkr_eye_bb_ry = 0.0f; dkr_eye_bb_rz = 0.0f;
    dkr_eye_bb_ux = 0.0f; dkr_eye_bb_uy = 1.0f; dkr_eye_bb_uz = 0.0f;
    dkr_eye_bb_p00 = 1.0f; dkr_eye_bb_p11 = 1.0f;
    dkr_eye_bb_p20 = 0.0f; dkr_eye_bb_p21 = 0.0f;
    dkr_eye_bb_cx = 0.0f; dkr_eye_bb_cy = 0.0f; dkr_eye_bb_cz = 0.0f;
    dkr_eye_bb_p22 = 0.0f;
    if (axes == NULL) {
        return;
    }
    dkr_eye_bb_rx = axes[0]; dkr_eye_bb_ry = axes[1]; dkr_eye_bb_rz = axes[2];
    dkr_eye_bb_ux = axes[3]; dkr_eye_bb_uy = axes[4]; dkr_eye_bb_uz = axes[5];
    if (axes[6] > 1e-6f) dkr_eye_bb_p00 = axes[6];
    if (axes[7] > 1e-6f) dkr_eye_bb_p11 = axes[7];
    /* The lens the card was AUTHORED through, which is not the eye's. Left
     * at zero when the publisher has none, and the rescale below stands down
     * rather than guessing. */
    dkr_eye_bb_flat_p00 = (axes[13] > 1e-6f) ? axes[13] : 0.0f;
    dkr_eye_bb_flat_p11 = (axes[14] > 1e-6f) ? axes[14] : 0.0f;
    dkr_eye_bb_p20 = axes[8]; dkr_eye_bb_p21 = axes[9];
    dkr_eye_bb_cx = axes[10]; dkr_eye_bb_cy = axes[11];
    dkr_eye_bb_cz = axes[12];
    dkr_eye_bb_p22 = axes[15];
    /* Armed whenever the axes deviate from rest identity: any rotation of
     * the pair, or any view DEPTH on either axis (head yaw or pitch). */
    if (fabsf(dkr_eye_bb_rx - 1.0f) > 1e-4f ||
        fabsf(dkr_eye_bb_ry) > 1e-4f || fabsf(dkr_eye_bb_rz) > 1e-4f ||
        fabsf(dkr_eye_bb_ux) > 1e-4f ||
        fabsf(dkr_eye_bb_uy - 1.0f) > 1e-4f ||
        fabsf(dkr_eye_bb_uz) > 1e-4f) {
        dkr_eye_bb_turn = true;
    }
}

/* First person hides the player's own kart, rider and wheels: every one of
 * their matrices registers with the SAME presentation owner address (the
 * racer's transform), so one address comparison at the primitive seam skips
 * the whole vehicle. Eye walks only -- the flat mirror keeps drawing the
 * kart, and vr_fp_hide_player() additionally stands down after the finish
 * line so the post-race camera shows the kart again. The game-side draw
 * cannot host this skip: the eye pass replays a captured list, so a skip at
 * authoring would hide the kart everywhere including the mirror. */
static const void *dkr_eye_hide_owner;

void gfx_dkr_set_eye_hide_owner(const void *transform) {
    dkr_eye_hide_owner = transform;
}

static bool dkr_eye_replay_hides_player(void) {
    return dkr_replay_pass && !dkr_replay_hud_only &&
           dkr_eye_hide_owner != NULL && vr_stereo_active() &&
           vr_fp_hide_player() &&
           rsp.active_slot >= 0 && rsp.active_slot < 3 &&
           rsp.shadow_matrix_valid[rsp.active_slot] &&
           rsp.shadow_matrix[rsp.active_slot].presentation_owner.valid &&
           rsp.shadow_matrix[rsp.active_slot].presentation_owner.address ==
               dkr_eye_hide_owner;
}

static bool dkr_eye_replay_skips_2d(void) {
    if (!dkr_replay_pass) {
        return false;
    }
    /* vr_stereo_active() is the begin/end-eye bracket and is FALSE during
     * the HUD walk (it runs after both eyes end), so the HUD mode keys on
     * its own flag -- gating it on the bracket left every filter dead and
     * the un-suppressed overlay stole the HUD to the back buffer while the
     * world landed opaque in the HUD texture. */
    if (dkr_replay_hud_only) {
        if (dkr_primitive_overlay_candidate) {
            dkr_hud_walk_drawn++;
            return false;
        }
        dkr_hud_walk_skipped++;
        return true;
    }
    return vr_stereo_active() && dkr_primitive_overlay_candidate;
}
...
static void dkr_sp_vertex(const Vertex *verts, int n, bool append,
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (rsp.billboard) {
            ...
            float oy = cy;
            /*
             * THE EYE WALK'S SPRITE COUNTER-ROLL, and its one legitimate home.
             *
             * A sprite's offsets are screen axes, so under head roll they stay
             * screen-upright while the world counter-rolls. Turning them by
             * world-up's on-screen direction puts them back on the world.
             *
             * It happens HERE, not at the matrix load, because "is this a
             * sprite" is only knowable once billboard mode is on: the racer's
             * head and the vehicle-part wheels share slot 2 with real sprites
             * and are ordinary MVPs. Rotating those tilted every racer's head
             * with the player's.
             *
             * The turn is in PIXEL space about the anchor, and a world-plane
             * substituted basis is excluded because it is already world-true.
             */
            /*
             * The gate is the published flat lens, NOT dkr_eye_bb_turn.
             * The turn flag arms only once the axes leave rest identity, so
             * it stood down exactly when the head was still -- and the size
             * error this path now corrects is at its full value standing
             * still. Head motion was never what made the cards wrong.
             */
            /*
             * MDKR_VR_BB=off stands the whole eye-space card rebuild down,
             * leaving every sprite exactly as the game authored it. Cards
             * then face the flat camera rather than the eye -- wrong, but
             * wrong in a KNOWN way. It exists because two different fixes
             * to this path both failed to change the reported stretching,
             * and the next thing worth knowing is not another correction
             * but whether this path draws those trees at all. One launch
             * answers it: if they still stretch with the rebuild off, the
             * palms are drawn somewhere else entirely (the world-plane
             * substituted population is the larger one -- bbworld 831k
             * against bbfallback 562k in the last session) and every hour
             * spent here is wasted.
             */
            if (dkr_replay_pass && !dkr_replay_hud_only &&
                vr_stereo_active() && !dkr_eye_bb_disabled() &&
                dkr_eye_bb_flat_p00 > 1e-6f && dkr_eye_bb_flat_p11 > 1e-6f &&
                rsp.active_slot >= 0 && rsp.active_slot < 3 &&
                !dkr_slot_world_billboard[rsp.active_slot]) {
                /* Re-project the authored screen offsets along BOTH card
                 * axes as world directions through this eye's lens,
                 * evaluated at THIS sprite's anchor (w term included).
                 * Identity at a level head; under head yaw/pitch/roll the
                 * card stays planted and thins or leans by true
                 * perspective instead of re-facing the eye as the head
                 * moves -- the same behavior the registered world-plane
                 * path ships, so the two sprite populations can never
                 * again disagree under head motion. */
                /*
                 * A card is only rebuilt from an anchor that is really IN
                 * FRONT of the eye. The old test, w > 1e-4, admits an anchor
                 * a hair in front: at w = 0.001 the projected NDC is in the
                 * thousands, every term below is scaled by it, and the quad
                 * is flung across the view. That is the "stretching all over
                 * the place" -- worst exactly where a palm sweeps past the
                 * camera, which is why hovercraft races (low, fast, trees
                 * close to the track) showed it most.
                 *
                 * Outside the bound the authored offsets stand: a card whose
                 * anchor is eight screens off-axis is clipped or invisible,
                 * so the flat card is both cheaper and safer than a rebuild
                 * amplified by a near-zero divide.
                 */
                float nx = 0.0f;
                float ny = 0.0f;
                bool anchor_sane = false;
                if (anchor->w > 1e-4f) {
                    nx = anchor->x / anchor->w;
                    ny = anchor->y / anchor->w;
                    anchor_sane = (fabsf(nx) < 8.0f && fabsf(ny) < 8.0f);
                }
                if (anchor_sane) {
                    /* CYLINDRICAL FACING, per anchor: the card's right axis
                     * swivels about the (fed, level) up axis toward this
                     * eye, so the card stays full-width from every
                     * direction instead of thinning edge-on when the head
                     * turns. The eye-to-anchor direction in eye-view space
                     * comes straight from the anchor's NDC. Degenerate
                     * cross falls back to the fed camera right. */
                    float brx = dkr_eye_bb_rx;
                    float bry = dkr_eye_bb_ry;
                    float brz = dkr_eye_bb_rz;
                    {
                        /* Anchor position in eye-view space (depth from the
                         * anchor's own w), and the facing ray FROM THE RIG,
                         * not from the head. */
                        const float ax_v =
                            anchor->w * (nx + dkr_eye_bb_p20) / dkr_eye_bb_p00;
                        const float ay_v =
                            anchor->w * (ny + dkr_eye_bb_p21) / dkr_eye_bb_p11;
                        const float az_v = -anchor->w;
                        const float ddx = ax_v - dkr_eye_bb_cx;
                        const float ddy = ay_v - dkr_eye_bb_cy;
                        const float ddz = az_v - dkr_eye_bb_cz;
                        float c0 = ddy * dkr_eye_bb_uz - ddz * dkr_eye_bb_uy;
                        float c1 = ddz * dkr_eye_bb_ux - ddx * dkr_eye_bb_uz;
                        float c2 = ddx * dkr_eye_bb_uy - ddy * dkr_eye_bb_ux;
                        const float len =
                            sqrtf(c0 * c0 + c1 * c1 + c2 * c2);
                        if (len > 1e-4f) {
                            /* No sign fix: see the shipped card path. */
                            brx = c0 / len;
                            bry = c1 / len;
                            brz = c2 / len;
                        }
                    ...
                    {
                        const float rcx = dkr_eye_bb_p00 * brx +
                                          brz * (dkr_eye_bb_p20 + nx);
                        const float rcy = dkr_eye_bb_p11 * bry +
                                          brz * (dkr_eye_bb_p21 + ny);
                        const float ucx = dkr_eye_bb_p00 * dkr_eye_bb_ux +
                                          dkr_eye_bb_uz *
                                              (dkr_eye_bb_p20 + nx);
                        const float ucy = dkr_eye_bb_p11 * dkr_eye_bb_uy +
                                          dkr_eye_bb_uz *
                                              (dkr_eye_bb_p21 + ny);
                        /*
                         * Divide by the lens that AUTHORED the offsets, not
                         * the one about to draw them. cx/cy are clip-space
                         * deltas from the flat 60-degree frustum; dividing
                         * them by the eye's much wider scales claims a
                         * bigger world size than the card ever had, which is
                         * why unregistered palms came out roughly 1.4x wide
                         * and 1.9x tall. The registered path has always done
                         * it this way (source_scale / cam->proj00).
                         */
                        const float wa = cx / dkr_eye_bb_flat_p00;
                        const float wb = cy / dkr_eye_bb_flat_p11;
                        ox = wa * rcx + wb * ucx;
                        oy = wa * rcy + wb * ucy;
                    }
                }
                dkr_eye_billboard_rolled++;
            }
            /*
             * THE SHIPPED CARD PROJECTION: a DKR sprite drawn as the WORLD
             * quad it stands for, through this eye, exactly.
             *
             * An authored card is built in the FLAT camera's clip space:
             * offsets scaled by the flat 60-degree lens and aligned to the
             * flat screen. Added unchanged to an eye-space anchor they are
             * wrong twice over. SIZE: the eye's lens is ~95 degrees, so the
             * same clip offset spans 1.4x the width and 1.9x the height it
             * was authored for -- every tree twice as tall, every balloon an
             * egg ("props stretching to the sky"). ORIENTATION: the offsets
             * are screen axes, so the card rolls with the head, and under a
             * pitched head it stays screen-vertical where the world's own
             * verticals fan out towards the edges -- a screen-glued card
             * "leans with your head" against the mesh beside it.
             *
             * So the card is rebuilt as what the game meant: a quad of the
             * authored WORLD size (offsets divided by the lens that authored
             * them), upright along the flat camera's up axis, swivelled about
             * that axis to face the camera RIG (never the per-frame head: a
             * lean must not swivel the scenery), and projected through the
             * eye lens PER VERTEX with its own w and z. The last point is the
             * whole difference from the opt-in rebuilds below, which project
             * the card's axes as a tangent plane at the anchor (first order
             * in offset/depth) and therefore fling the far corners of any
             * large card close to the eye -- a palm sweeping past the kart
             * -- which read as stretching and tearing. A vertex carrying its
             * own w is the exact perspective of a world quad at any distance;
             * corners that fall behind the eye are clipped like any other
             * geometry instead of exploding.
             *
             * MDKR_VR_BBCARD=off returns to raw authored cards (screen-glued,
             * oversize, tear-free) for an A/B. Inert in the flat lane, the
             * HUD walk, the eye-debug ground-truth mode (no authoring lens
             * is published there) and on slots the opt-in world-plane
             * rebuild already substituted.
             */
            if (dkr_replay_pass && !dkr_replay_hud_only &&
                vr_stereo_active() && dkr_eye_card_enabled() &&
                dkr_eye_bb_disabled() &&
                rsp.active_slot >= 0 && rsp.active_slot < 3 &&
                !dkr_slot_world_billboard[rsp.active_slot] &&
                dkr_eye_bb_flat_p00 > 1e-6f && dkr_eye_bb_flat_p11 > 1e-6f &&
                dkr_eye_bb_p00 > 1e-6f && dkr_eye_bb_p11 > 1e-6f &&
                anchor->w > 1e-3f) {
                /* The card's extent in WORLD units: undo the authoring lens.
                 * (mtxf_billboard bakes the raster aspect into y and the
                 * port's billboard correction bakes the window's lens into
                 * both, so these two divides are the whole story.) */
                const float tx = cx / dkr_eye_bb_flat_p00;
                const float ty = cy / dkr_eye_bb_flat_p11;
                /* The anchor in eye-view space (from its own clip position;
                 * w is the view depth because P[2][3] is -1). */
                const float aw = anchor->w;
                const float ax_v =
                    (anchor->x + dkr_eye_bb_p20 * aw) / dkr_eye_bb_p00;
                const float ay_v =
                    (anchor->y + dkr_eye_bb_p21 * aw) / dkr_eye_bb_p11;
                const float az_v = -aw;
                /* Card right: the flat camera's right, swivelled about the
                 * flat camera's up to face the rig. Degenerate (anchor on
                 * the rig's vertical) keeps the camera right. */
                float brx = dkr_eye_bb_rx;
                float bry = dkr_eye_bb_ry;
                float brz = dkr_eye_bb_rz;
                {
                    ...
                    float c1 = ddz * dkr_eye_bb_ux - ddx * dkr_eye_bb_uz;
                    float c2 = ddx * dkr_eye_bb_uy - ddy * dkr_eye_bb_ux;
                    const float len = sqrtf(c0 * c0 + c1 * c1 + c2 * c2);
                    /* (anchor - rig) x up IS screen-right as seen from the
                     * rig towards the anchor: +x for every anchor in front,
                     * and the correctly turned right for one behind. No
                     * sign fix towards camera-right -- that flipped cards
                     * whose anchor crossed the camera plane under a big
                     * head turn (a mirrored palm beside the kart). */
                    if (len > 1e-4f) {
                        brx = c0 / len;
                        bry = c1 / len;
                        brz = c2 / len;
                ...
                /* Both axes through the eye lens as full clip DIRECTIONS
                 * (x, y, z, w) -- w = -z_view, the row P[2][3] = -1. */
                {
                    const float dx_v = tx * brx + ty * dkr_eye_bb_ux;
                    const float dy_v = tx * bry + ty * dkr_eye_bb_uy;
                    const float dz_v = tx * brz + ty * dkr_eye_bb_uz;
                    ox = dx_v * dkr_eye_bb_p00 + dz_v * dkr_eye_bb_p20;
                    oy = dy_v * dkr_eye_bb_p11 + dz_v * dkr_eye_bb_p21;
                    d->x = anchor->x + ox;
                    d->y = anchor->y + oy;
                    d->z = anchor->z + dz_v * dkr_eye_bb_p22;
                    d->w = aw - dz_v;
                }
                dkr_eye_card_vertices++;
            } else {
                d->x = anchor->x + ox;
                d->y = anchor->y + oy;
                d->z = anchor->z + cz;
...
#ifdef ENABLE_VR
    /*
     * An EYE is not a television. The flat frame fits its world into a
     * letterboxed sub-rectangle of the surface (presentation, or the 4:3
     * safe region) and the GL scissor cuts hard at its edge -- correct for
     * a monitor, and in a headset it is a guillotine across the field of
     * view: geometry that the eye frustum legitimately sees is discarded
     * because the FLAT framing says the surface ends there. It shows up
     * first on whatever is big and tall near the edges, which is trees and
     * props (and it only became obvious once sprites were sized correctly
     * in world space -- small sprites never reached the cut).
     *
     * The eye pass therefore renders full-bleed: the eye projection already
     * defines the field, and the whole eye target IS the field. 2D never
     * reaches here in an eye walk (it is filtered out) and the HUD walk
     * keeps the flat framing, which is what its quad wants.
     */
    if (dkr_replay_pass && !dkr_replay_hud_only && vr_stereo_active()) {
        return layout.drawable;
    }
#endif
    switch (draw_space) {
...
static void dkr_dp_fill_rectangle(int32_t ulx, int32_t uly, int32_t lrx, int32_t lry) {
     ...
     * RAW SETCIMG/SETZIMG values rather than the resolved pointers: DKR clears
     * the depth buffer by pointing SETCIMG at the z-buffer segment (so both
     * values are e.g. 0x02000000), and on wasm32 those segment values resolve to
     * NULL -- a resolved-pointer compare would then draw the clear as a white
     * fill over the scene (M3b). values are width-independent and identical on
    ...
    /* Restore the SAVED suppression state, not false: the VR eye replay
     * suppresses the overlay for its whole walk, and the very first scene
     * clear used to un-suppress it. */
    bool saved_overlay_suppressed = dkr_output_overlay_suppressed;
    /*
    ...
    if (promoted_to_fullbleed) {
        dkr_output_overlay_suppressed = saved_overlay_suppressed;
...
static void dkr_run_dl(Gfx *cmd, int depth, int limit) {
    ...
    for (;;) {
        ...
        case G_MTX: {  /* gSPMatrixDKR -- load slot and select it */
            ...
            bool vr_billboard_substituted = false;
            ...
            /* VR eye walk: billboards become WORLD sprite planes composed
             * with the eye VP (mdkr_camera_vr_billboard_matrix). Keyed on
             * the eye bracket alone, NOT on object-alpha state: the
             * display-rate eye pass re-walks with interpolation armed
             * (object_alpha_valid true), and gating on that handed those
             * frames the flat interpolated billboard path below -- the
             * authored screen-glued basis, turning with the head. */
            if (world_matrix && dkr_replay_pass && !dkr_replay_hud_only &&
                vr_stereo_active()) {
                extern int mdkr_camera_vr_billboard_matrix(
                    const GfxPresentationMatrixOwner *owner, int viewport,
                    void *out_mtx);
                /* Refusal census: which gate eats the substitution. One
                 * line every 4096 attempts, so a silent zero can never hide
                 * again (this path shipped dead once because the packet
                 * lookup's preconditions were assumed, not measured). */
                static uint64_t vrbbAttempt, vrbbFound, vrbbClass, vrbbBuilt;
                bool found = false, classOk = false, built = false;
                vrbbAttempt++;
                found = gfx_presentation_packet_lookup_matrix_observed(
                    matrix_identity, ma, &packet_binding);
                if (found) {
                    vrbbFound++;
                    classOk = packet_binding.owner.matrix_class ==
                              GFX_PRESENTATION_MATRIX_BILLBOARD;
                }
                /* MDKR_VR_BBWORLD=off stands the WORLD-PLANE billboard
                 * substitution down, the same way MDKR_VR_BB=off stands the
                 * fallback rebuild down. Between the two switches every
                 * sprite in the game can be returned to its authored form,
                 * which is how the stretching gets localised to a
                 * population instead of guessed at a line. This is the
                 * larger population (bbworld 831k vs bbfallback 562k) and
                 * the only one left once the fallback was ruled out. */
                if (classOk && !dkr_eye_bbworld_disabled()) {
                    vrbbClass++;
                    built = mdkr_camera_vr_billboard_matrix(
                ...
                if (built) {
                    ...
                    vr_billboard_substituted = true;
                    dkr_replay_billboard_matrix_hits++;
                }
                if ((vrbbAttempt & 0xfffu) == 1u) {
                    GfxPresentationPacketStats ps;
                    gfx_presentation_packet_get_stats(&ps);
                    fprintf(stderr,
                            "[vrbb] attempts=%llu found=%llu class=%llu "
                            "built=%llu (packet reg=%llu frozen=%d)\n",
                            (unsigned long long)vrbbAttempt,
                            (unsigned long long)vrbbFound,
                            (unsigned long long)vrbbClass,
                            (unsigned long long)vrbbBuilt,
                            (unsigned long long)ps.matrix_registrations,
                            gfx_presentation_packet_frozen() ? 1 : 0);
                }
            }
            if (world_matrix && dkr_replay_pass &&
                dkr_replay_object_alpha_valid) {
            ...
            if (billboard_binding_found && !vr_billboard_substituted) {
                /* The eye walk's world-plane rebuild already owns this slot;
                 * computing the flat screen-space override under it would be
                 * wasted work and a double-counted hit. */
            ...
            if (vr_billboard_substituted) {
                /* The eye-walk world-plane rebuild already decoded slot 2;
                 * every fallback below would overwrite it with the authored
                 * screen-glued bytes. */
            } else if (billboard_overridden) {
                if (packet_binding.stale &&
                    dkr_replay_object_alpha_numerator == 0u) {
            ...
            if (slot >= 0 && slot < 3) {
                /*
                 * Record only. The counter-roll itself moved to the BILLBOARD
                 * VERTEX path (see G_VTX, rsp.billboard), because this site
                 * cannot tell a sprite basis from an ordinary MVP and slot 2
                 * carries both.
                 *
                 * Four sites push slot 2 and only ONE is a clip-space sprite:
                 * render_sprite_billboard's billboard branch (camera.c) and
                 * render_ortho_triangle_image are sprites; the vehicle-part
                 * branch (wheels, propellers, fans) and mtx_head_push (the
                 * RACER'S HEAD) upload full world x view-projection matrices
                 * and deliberately never enable billboard mode. Rotating
                 * those rotated the heads and the wheels with the player's
                 * head roll -- reported, exactly, as "trees and all racers
                 * tilt their heads too".
                 *
                 * Billboard mode is switched on by a MOVEWORD after this
                 * load, so rsp.billboard is false for all four producers here
                 * and cannot be the gate. The vertex path is the one place
                 * that knows, by construction, that it is drawing a sprite.
                 */
                dkr_slot_world_billboard[slot] = vr_billboard_substituted;
            }
            if (draw_space != G_MTX_DKR_SPACE_INHERIT) {
        ...
        case G_TRIN: {  /* gSPPolygon */
            ...
            if (t) {
                ...
                dkr_begin_primitive(
                    rsp.draw_space != G_MTX_DKR_SPACE_WORLD);
                if (!dkr_eye_replay_skips_2d() &&
                    !dkr_eye_replay_hides_player()) {
        ...
        case G_TEXRECTFLIP: {
            ...
            if (!dkr_eye_replay_skips_2d()) {
                dkr_dp_texture_rectangle(ulx, uly, lrx, lry, tile, uls, ult,
                                         dsdx, dtdy,
...
/* MDKR_VR_MATRIXPROBE: temporary stereo diagnosis. Prints the registry
 * counts at freeze time and one sample address on each side, so an empty
 * snapshot and a key mismatch read differently instead of both being
 * "hits=0". */
static int dkr_vr_matrix_probe_enabled(void) {
    static int enabled = -1;
    if (enabled < 0) {
        enabled = getenv("MDKR_VR_MATRIXPROBE") != NULL;
    }
    return enabled;
}
...
void gfx_end_frame(void) {
    ...
    if (present_sched_replay_armed()) {
        ...
        if (dkr_vr_matrix_probe_enabled()) {
            fprintf(stderr,
                    "[vrprobe] frame=%d freeze: live_at_freeze=%zu frozen=%zu "
                    "pkt_frozen=%d\n",
                    dkr_frame_index, gfx_shadow_matrix_live_count(),
                    gfx_shadow_replay_frozen_matrix_count(),
                    gfx_presentation_packet_frozen() ? 1 : 0);
        }
    } else if (dkr_vr_matrix_probe_enabled()) {
        static int skip_logged = 0;
        if (!skip_logged) {
            skip_logged = 1;
            fprintf(stderr,
                    "[vrprobe] frame=%d freeze SKIPPED (replay not armed)\n",
                    dkr_frame_index);
        }
    }
...
#ifdef ENABLE_VR
    if (dkr_replay_hud_only) {
        /* The HUD pass retargets the walk at a transparent render-res
         * texture; start_frame bound the eye target (vr_stereo_active), so
         * rebind here, before any state lands. GL-only, like the eye path. */
        extern bool gfx_opengl_vr_hud_begin(void);
        if (!gfx_opengl_vr_hud_begin()) {
            goto replay_cleanup;
        }
    }
#endif
    gfx_dkr_reset_interpreter_state();
    /* The eye pass renders the WHOLE list into the eye target at render
     * scale. Left enabled, the output overlay switches the HUD portion (and
     * every draw after it) to the OUTPUT-resolution overlay target mid-pass
     * and remaps regions through output dimensions -- measured as the
     * in-race eye rendering its world through a 640x640 viewport while the
     * flat frame used 1280x1280, which presented as a magnified crop no
     * separation change could move. */
    if (vr_stereo_active() || dkr_replay_hud_only) {
        dkr_output_overlay_suppressed = true;
    }
...
#ifdef ENABLE_VR
    if (dkr_replay_hud_only) {
        extern void gfx_opengl_vr_hud_end(void);
        gfx_opengl_vr_hud_end();
    }
#endif
    /* The HUD walk skips the frame close: its start_frame took the ordinary
     * scene-target branch, and end_frame would re-resolve that STALE scene
     * over the back buffer -- wiping the HUD off the desktop mirror that is
     * waiting to present. Everything it drew is already flushed into its own
     * texture. */
    if (!dkr_replay_hud_only) {
...
uint64_t gfx_dkr_eye_card_vertex_count(void) {
    /* Running count of billboard vertices the shipped card projection
     * rebuilt (world quads through the eye lens). Beside bbfallback/bbworld
     * in the eye log: with both rebuilds off this is the live path, and a
     * zero here with stereo running means no authoring lens reached it. */
    return dkr_eye_card_vertices;
}

uint64_t gfx_dkr_eye_billboard_rolled_count(void) {
    /* Running count of billboard vertices that took the vertex-path
     * counter-roll (the UNREGISTERED-sprite fallback). Read beside bbworld
     * in the periodic eye log: the two counters together say which of the
     * two sprite paths the scenery on screen is actually riding. */
    return dkr_eye_billboard_rolled;
}

bool gfx_dkr_replay_walk_vr_hud(void) {
#ifdef ENABLE_VR
    /* The VR HUD pass: same retained list, 2D candidates only, into the
     * transparent HUD texture (see dkr_replay_hud_only). No overrides -- 2D
     * draws carry their own matrices. */
    bool ok;
    dkr_replay_hud_only = true;
    ok = gfx_dkr_replay_walk_impl(NULL, 0, false, true, 0u, 1u);
    dkr_replay_hud_only = false;
    if (dkr_vr_matrix_probe_enabled()) {
        static uint64_t lastDrawn, lastSkipped, lastRolled;
        fprintf(stderr,
                "[vrprobe] hud walk: ok=%d drawn=%llu skipped=%llu "
                "bbrolled=%llu (delta)\n",
                ok ? 1 : 0,
                (unsigned long long)(dkr_hud_walk_drawn - lastDrawn),
                (unsigned long long)(dkr_hud_walk_skipped - lastSkipped),
                (unsigned long long)(dkr_eye_billboard_rolled - lastRolled));
        lastDrawn = dkr_hud_walk_drawn;
        lastSkipped = dkr_hud_walk_skipped;
        lastRolled = dkr_eye_billboard_rolled;
    }
    return ok;
#else
    return false;
...
bool gfx_dkr_replay_walk(
    const GfxShadowReplayViewProjection *overrides, size_t override_count) {
    if (dkr_vr_matrix_probe_enabled() && dkr_eye_billboard_roll != 0.0f) {
        static uint64_t lastRolled;
        bool ok = gfx_dkr_replay_walk_impl(overrides, override_count, false,
                                           true, 0u, 1u);
        fprintf(stderr, "[vrprobe] eye walk: bbrolled=%llu (delta)\n",
                (unsigned long long)(dkr_eye_billboard_rolled - lastRolled));
        lastRolled = dkr_eye_billboard_rolled;
        return ok;
    }
