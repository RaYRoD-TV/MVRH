...
#ifdef __ANDROID__
/*
 * WHERE A HEADSET BUILD KEEPS THE PLAYER'S THINGS.
 *
 * Every other platform falls through to the relative path "save", resolved
 * against the working directory the game was launched from. An Android app has
 * no such directory: it starts in "/", which is read-only, so the eeprom and
 * the VR settings both failed to write and every in-headset tuning reverted on
 * quit, with only "[SAVE] could not create save: Read-only file system" in the
 * log to say so.
 *
 * The app's own EXTERNAL files directory is the answer rather than the internal
 * one, for three reasons: it is app-owned so it needs no permission, it is the
 * same folder the hub's Send to Quest pushes the ROM into, and it is reachable
 * over the cable, which is what lets the hub back a player's save up before it
 * replaces the app. Declared rather than included for the same reason
 * SDL_GetPrefPath above is: this module stays free of SDL headers so its
 * packaged-path unit test can supply its own provider.
 */
extern const char *SDL_AndroidGetExternalStoragePath(void);

static int android_files_subpath(char *output, size_t output_size,
                                 const char *leaf) {
    const char *base = SDL_AndroidGetExternalStoragePath();
    if (base == NULL || base[0] == '\0') {
        set_last_error("Android external files directory is unavailable", NULL);
        return 0;
    }
    return path_join(output, output_size, base, leaf);
}
#endif
...
#ifdef __ANDROID__
    return android_files_subpath(output, output_size, "mdkr64.ini");
#else
    if (s_packaged) {
        return s_pref_ready &&
...
#ifdef __ANDROID__
    return android_files_subpath(output, output_size, "save");
#else
    if (s_packaged) {
