...
/* backtrace(): glibc and macOS only. Absent on wasm and on Windows, and a trap
 * on Android, where NDK 27 ships execinfo.h but hides the declarations below
 * API 33: the include succeeds and the call is undeclared at the link. Android
 * writes its own symbolized tombstone to logcat on a fault, which is the better
 * trace anyway. */
#include <execinfo.h>
#endif
#endif
/* On Android SDLActivity's native thread calls SDL_main, and SDL_main.h only
 * renames this file's main() to that when SDL_MAIN_HANDLED is NOT set. Every
 * other platform keeps the plain int main() this port has always had. */
#ifndef __ANDROID__
...
static void mdkr_crash_handler(int sig) {
    ...
    /* No usable execinfo here (see the include guard). debuggerd writes the full
     * symbolized tombstone to logcat on a fault, so this only has to name the
     * signal in the shape every other platform prints it. */
    (void) bt;
    fprintf(stderr, "\n[CRASH] signal %d - backtrace: see the logcat tombstone\n", sig);
#else
...
static void print_usage(const char *program) {
    printf("Usage: %s [ROM] [options]\n"
           ...
           "  --vr                       request VR (OpenXR; implies the GL backend)\n"
           "  --vrfake                   stereo eye loop with no headset (diagnostic)\n"
           "  --novr                     never enable VR, even with a headset present\n",
           program);
}
...
int main(int argc, char **argv) {
    ...
    SDL_SetMainReady();
#ifdef __ANDROID__
    /* Before anything prints. An Android app's stdout and stderr go nowhere,
     * so without this the boot banner, the VR log and the crash marker are all
     * thrown away and a run that failed looks like a run that never started. */
    mdkr_android_log_stdio();
#endif
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--rom") == 0 && i + 1 < argc) {
            ...
            wantVr = 1;
        } else if (strcmp(argv[i], "--vrfake") == 0) {
            wantVr = 1;
            wantVrFake = 1;
...
#ifdef __ANDROID__
    /* There is no command line and no working directory to drop a ROM beside,
     * so the headset build goes and finds it: the app's own files folder first,
     * which is where the hub's Send to Quest writes, then shared storage. A
     * NULL here falls through to the DEFAULT_ROM below and fails with the
     * loader's own message, which android_boot.c has already qualified with
     * where it looked. */
    if (!romPath) romPath = mdkr_android_find_rom();
#endif
    ...
    /*
     * VR (OpenXR), decided before the window exists because it picks the
     * window's backend (the decision itself lives with the backend resolver:
     * mdkr_vr_startup_decide in platform_sdl_min.c). --vr forces the request,
     * --novr vetoes even auto-detection, and with neither the same exe
     * auto-enables when a headset answers a lightweight probe (throwaway
     * OpenXR instance, no GL needed; a runtime with the headset asleep
     * answers no and the game stays flat). Session boot itself is lazy, on
     * the first rendered frame (vr_begin_frame), and failing there still
     * leaves the flat game running. The launcher path makes the same call
     * with no flags before it creates its window (main_app.cpp), so a bare
     * double-click auto-detects too.
     */
    if (wantVrFake) {
        /* Stereo geometry and eye-pass cost, measured on a desk: the full eye
         * loop against a synthesized head pose, with no OpenXR runtime and no
         * headset involved and nothing submitted anywhere. This is how the eye
         * dump is captured (MDKR_VR_EYEDUMP), so the pair being measured comes
         * out of the same code the headset runs. */
        vr_request_fake();
    }
    mdkr_vr_startup_decide(wantVr, noVr);
#ifndef ENABLE_VR
    /* A --vr user deserves the truth about this binary. */
    if (wantVr) {
        fprintf(stderr, "[mdkr64] --vr: this build has no VR support "
                        "(rebuild with -DMDKR_VR=ON); running flat.\n");
    }
#endif
