...
#define SET_DEFAULT_TEXT(KEY, VALUE) \
    ...
    /*
     * TRIGGERS DRIFT, SHOULDERS HONK -- the modern-pad layout, swapped from
     * the N64's literal one.
     *
     * Drift is R_TRIG in the game itself (racer.c: velocity + R_TRIG starts
     * the slide), and Z fires a balloon or honks the horn when you have
     * none. Every racing game built since the N64 puts the slide on a
     * trigger and the analog pull is what a slide wants, so both triggers
     * now drift and both shoulders take Z.
     *
     * N64 L is left unbound by default. It does nothing in a race -- only
     * debug and cheat-entry combos read it -- and it is one rebind away for
     * anyone who wants it back.
     *
     * NOTE these are DEFAULTS: a saved config pins whatever it already has,
     * so an existing player keeps the old layout until they reset input.
     */
    SET_DEFAULT_TEXT(MDKR_INPUT_CONTROLLER_LEFT_SHOULDER, "z");
    SET_DEFAULT_TEXT(MDKR_INPUT_CONTROLLER_RIGHT_SHOULDER, "z");
    ...
    SET_DEFAULT_TEXT(MDKR_INPUT_CONTROLLER_LEFT_TRIGGER, "r");
    SET_DEFAULT_TEXT(MDKR_INPUT_CONTROLLER_RIGHT_TRIGGER, "r");
