...
/* VR startup decision, once per process, BEFORE the first backend resolve and
 * before any window exists. Grants VR (and selects the GL backend, the only
 * one the OpenXR/WGL binding rides) when want_vr is set or a headset answers
 * the probe, unless no_vr vetoes. Idempotent; later calls are no-ops. The CLI
 * engine path passes its --vr/--novr flags; the launcher path passes 0,0 so a
 * bare double-click auto-detects. Without MDKR_VR this compiles to nothing. */
void mdkr_vr_startup_decide(int want_vr, int no_vr);

/* VR stereo: re-draw the finished display list once per eye.
 *
 * Called from the frame driver IMMEDIATELY after the real walk, not from
 * present: the matrix registry the eye pass substitutes into keys by Mtx
 * pointer, and DKR builds the next list into that same memory while the
 * current one is being presented, so by present time every entry is a dead
 * tenant and nothing can be substituted. Opens the XR frame if it is not
 * already open. Inert without a live session or without MDKR_VR_STEREO. */
void platform_vr_render_eyes(void);

/* The display-rate sibling, called from the host-opportunity subloop for
 * every sub-tick present: the same eye pass composed from the INTERPOLATED
 * camera pair at alpha numerator/denominator, walked with object
 * interpolation armed. This is what turns one stereo frame per 30 Hz game
 * tick into one per headset refresh -- the compositor stops inventing three
 * of every four frames, which is what props shearing under head motion and
 * first-person jank were. Inert without a live session. */
void platform_vr_render_eyes_interpolated(uint64_t numerator,
                                          uint64_t denominator);

/* VR settings persistence: a small key=value file beside the saves.
 * Saved when the VR OPTIONS page closes; loaded once at VR startup BEFORE
 * the MDKR_VR_* env knobs are read, so an env pin still wins its launch.
 * No-ops on flat builds and flat runs. */
void mdkr_vr_settings_save(void);
void mdkr_vr_settings_load(void);

/* Hold-to-look-behind (keyboard C, VR Y). The game's view builder reads it;
 * the original game has no rear view of its own. */
int mdkr_look_behind_held(void);

/* True on the frame the PHYSICAL right trigger goes down. The pause menu's
 * options shortcut reads this instead of an N64 button, so rebinding R can
 * never move the shortcut off the trigger it is named after. */
