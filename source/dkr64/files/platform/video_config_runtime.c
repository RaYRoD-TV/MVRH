...
/*
 * A CHANGED DEFAULT HAS TO MIGRATE, or a saved config pins the old one
 * forever. The pad layout moved so the TRIGGERS drift and the SHOULDERS
 * honk; anyone who had launched before was carrying an mdkr64.ini that said
 * shoulders l/r and triggers z/z, so the new defaults reached nobody -- the
 * change looked like it simply did not work, which is exactly how it was
 * reported.
 *
 * The migration only fires on a config still sitting on the EXACT old
 * default four. A player who bound any of them deliberately does not match,
 * and their choice is left alone: this moves people off a default they never
 * chose, and never overwrites one they did.
 */
static void mdkr_video_config_migrate_pad_layout(MdkrVideoConfig *config) {
    static const struct { MdkrVideoKey key; const char *was; const char *now; }
    kMoves[] = {
        { MDKR_INPUT_CONTROLLER_LEFT_SHOULDER,  "l", "z" },
        { MDKR_INPUT_CONTROLLER_RIGHT_SHOULDER, "r", "z" },
        { MDKR_INPUT_CONTROLLER_LEFT_TRIGGER,   "z", "r" },
        { MDKR_INPUT_CONTROLLER_RIGHT_TRIGGER,  "z", "r" },
    };
    size_t i;
    ...
    for (i = 0; i < sizeof(kMoves) / sizeof(kMoves[0]); i++) {
        if (strcmp(config->values[kMoves[i].key].text, kMoves[i].was) != 0) {
            return;   /* not the old layout -- somebody chose this; leave it */
        }
    }
    for (i = 0; i < sizeof(kMoves) / sizeof(kMoves[0]); i++) {
        snprintf(config->values[kMoves[i].key].text,
                 sizeof(config->values[kMoves[i].key].text), "%s",
                 kMoves[i].now);
    }
    fprintf(stderr,
            "[input] pad layout updated: triggers drift, shoulders honk "
            "(rebind either in the options if you prefer the old one)\n");
}
...
void mdkr_video_config_init(int argc, char *const *argv) {
    ...
    mdkr_video_config_migrate_pad_layout(&s_video);
...
/*
 * VR presentation floor: raise the LIVE render scale for THIS process only,
 * never touching the config file -- the eye buffers upsample
 * gfx_current_dimensions into the headset swapchain, and the flat default's
 * 1280x960 reads soft against a ~3k-tall eye image. The floor yields to any
 * value the player chose themselves (env, CLI, or an equal-or-higher
 * setting), and a later Restart & Apply resolves back to the player's own
 * configuration because s_desired_video is deliberately left alone.
 */
/*
 * Set a LIVE setting for this process only (never persisted), in EITHER
 * direction. Separate from the floor below because the VR render scale is
 * derived from the headset and the window and must be able to come DOWN
 * when the window grows -- a one-way floor there is how a 4x literal
 * survived a window resize into a 16x-pixel allocation the GL driver
 * refused (crash inside nvoglv64 on the first draw).
 */
void mdkr_video_config_vr_set_live(MdkrVideoKey key, float value) {
    const MdkrVideoValue *cur = &s_video.values[key];
    const MdkrVideoSchema *schema = mdkr_video_schema(key);
    char text[32];

    if (cur->number > value - 0.01f && cur->number < value + 0.01f) {
        return;                     /* already there */
    }
    if (cur->source == MDKR_VIDEO_SOURCE_ENV ||
        cur->source == MDKR_VIDEO_SOURCE_CLI) {
        return;                     /* the player pinned it this launch */
    }
    snprintf(text, sizeof(text), "%g", value);
    if (mdkr_video_config_set(&s_video, key, text,
                              MDKR_VIDEO_SOURCE_RUNTIME)) {
        mdkr_video_config_publish();
        fprintf(stderr,
                "[vr] %s set to %g for the headset "
                "(live only; your saved config is untouched)\n",
                schema != NULL ? schema->name : "setting", (double)value);
    }
}

void mdkr_video_config_vr_set_live_text(MdkrVideoKey key, const char *value) {
    const MdkrVideoValue *cur = &s_video.values[key];
    const MdkrVideoSchema *schema = mdkr_video_schema(key);

    /* Stored text is canonicalised by mdkr_video_config_set, and the VR
     * callers pass canonical values, so plain equality is the early-out. */
    if (value == NULL || strcmp(cur->text, value) == 0) {
        return;                     /* already there */
    }
    if (cur->source == MDKR_VIDEO_SOURCE_ENV ||
        cur->source == MDKR_VIDEO_SOURCE_CLI) {
    ...
    if (mdkr_video_config_set(&s_video, key, value,
                              MDKR_VIDEO_SOURCE_RUNTIME)) {
        mdkr_video_config_publish();
        fprintf(stderr,
                "[vr] %s set to %s for the headset "
                "(live only; your saved config is untouched)\n",
                schema != NULL ? schema->name : "setting", value);
    }
}

void mdkr_video_config_vr_floor(MdkrVideoKey key, float value) {
    const MdkrVideoValue *cur = &s_video.values[key];
    const MdkrVideoSchema *schema = mdkr_video_schema(key);
    char text[32];
    ...
    if (mdkr_video_config_set(&s_video, key, text,
        ...
        mdkr_video_config_publish();
        fprintf(stderr,
                "[vr] %s floored to %g for the headset "
                "(live only; your saved config is untouched)\n",
                schema != NULL ? schema->name : "setting", (double)value);
    }
}
