...
#include "vr/vr.h"      /* vr_is_requested: a chosen ROM plays straight away */
...
LauncherAction Launcher::draw(AppHost &host) {
    ...
    /*
     * PICKING THE ROM IS THE PLAY PRESS, in VR.
     *
     * A headset session already skips this window entirely once a ROM is
     * remembered (main_app.cpp). The one run that still opens it is the very
     * first, and there the player used to choose their ROM and then have to
     * find and press Play as well -- on a monitor they are not looking at,
     * through a window the headset runtime keeps unfocused. Two steps where
     * the second one carries no decision: the answer to "which ROM" is the
     * whole of what the launcher needed.
     *
     * So a freshly accepted choice goes straight into the same final check
     * the Play button runs, and the boot follows from its result. Nothing
     * skips verification -- the ROM is still fully validated before a byte
     * of it is booted.
     *
     * VR only, deliberately: on the desktop the launcher is a window a person
     * chose to look at, with Settings and Diagnostics beside the ROM panel,
     * and taking it away the instant a file validates would be taking away
     * their reason for opening it.
     */
    const bool justChose = state_.romSelectionJustAccepted;
    state_.romSelectionJustAccepted = false;
    if (justChose && vr_is_requested() && !state_.romPath.empty() &&
        state_.romInfo.valid) {
        std::fprintf(stderr,
                     "[app] VR: ROM chosen, going straight into the game.\n");
        preparePlay(state_);
    }
