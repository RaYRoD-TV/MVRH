...
#include "platform_os.h"   /* mdkr_vr_startup_decide */
#include "vr/vr.h"         /* vr_is_requested: skip the launcher in VR */
#include "dev_tools.h"
#include "diag_log.h"
...
int runEngineSession(AppHost &host, SessionRuntime &session,
    ...
    if (!sessionRom.empty() && AppConfig::get("rom_path", "") != sessionRom) {
        AppConfig::setAndSave("rom_path", sessionRom);
    }
...
int main(int argc, char **argv) {
    ...
    /* VR auto-detection for the double-click flow: no flags to pass here, so a
     * present-and-awake headset simply grants VR (and the GL backend) before
     * the one launcher/game window is created. The CLI path makes this same
     * call with its --vr/--novr flags in main_pc.c; whichever runs first
     * decides, and without MDKR_VR it compiles to nothing. */
    mdkr_vr_startup_decide(0, 0);
    ...
    std::string restartRom;
    /*
     * VR: boot the remembered ROM instead of opening the launcher.
     *
     * The launcher is a DESKTOP window, and a player wearing a headset cannot
     * reach it -- the game window is never focused under a headset, and they
     * cannot see the monitor at all. Asking them to pick the same ROM again,
     * blind, every launch is the exact "documented ritual" the bring-your-own
     * -ROM rule exists to prevent. So when VR was granted at startup and the
     * launcher already remembers a readable ROM, take the same warm-up-then-
     * boot path Restart & Apply uses.
     *
     * Deliberately VR-only: the flat launcher keeps its shipped behaviour, and
     * a first VR run with nothing remembered still opens the launcher so the
     * ROM can be chosen once on the monitor.
     */
    std::string vrRememberedRom;
    if (vr_is_requested() && !AppRestart_pendingGame()) {
        vrRememberedRom = AppConfig::get("rom_path", "");
        if (!vrRememberedRom.empty()) {
            FILE *probe = mdkr_fopen_utf8(vrRememberedRom.c_str(), "rb");
            if (probe != nullptr) {
                std::fclose(probe);
            } else {
                std::fprintf(stderr,
                             "[app] VR: remembered ROM is unreadable, opening "
                             "the launcher: %s\n", vrRememberedRom.c_str());
                vrRememberedRom.clear();
            }
        }
    }
    ...
    if (!vrRememberedRom.empty()) {
        std::fprintf(stderr,
                     "[app] VR: booting the remembered ROM without the "
                     "launcher: %s\n", vrRememberedRom.c_str());
        exitCode = runRestartSession(
            host, launcher, session, &transition, vrRememberedRom,
            &bootRecoveryMessage);
    } else if (AppRestart_pendingGame() && AppRestart_consumeGame(restartRom)) {
        std::fprintf(stderr, "[app] Restart & Apply handoff accepted\n");
        exitCode = runRestartSession(
