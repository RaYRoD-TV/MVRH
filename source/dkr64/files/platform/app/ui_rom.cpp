...
static void applySelectionResult(LauncherState &s,
    ...
    if (!AppConfig::persistResultApplied(persist)) {
        if (!s.romInfo.valid) {
            ...
            g_changing = false;
            /* Playable, just not remembered: the player still chose it, so it
             * still counts as "and now play it". */
            s.romSelectionJustAccepted = true;
            return;
        }
        s.romCandidatePath = candidatePath;
        s.romCandidateInfo = candidateInfo;
    ...
    g_changing = false;
    /* The player handed us a ROM and it checked out. Only this path sets it;
     * the remembered ROM returns above, so a normal startup never looks like
     * a fresh choice. */
    s.romSelectionJustAccepted = true;
}
