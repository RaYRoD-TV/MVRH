...
static int onRender(void) {
    ...
    beginImGuiFrame();
    /*
     * The view-mode toast is GONE. It was an ImGui panel floating over the
     * game in a font and frame the game does not own -- a debug readout
     * wearing a player-facing job, and the one piece of chrome on screen
     * that could never pass for Diddy Kong Racing.
     *
     * Nothing replaces it here on purpose. The view mode is already legible
     * the instant it changes -- the camera moves -- and the VIEW row in the
     * options page names it in the game's own font. If a confirmation is
     * ever wanted it belongs there, drawn with the game's text, not in an
     * overlay layer that exists for diagnostics.
     */
