...
#include "fs_utf8.h"
#include "audio.h"
#include "vr/vr.h"     /* vr_is_active; inline no-op on flat builds */
...
static int dkr_fs_mkdir_private(const char *path) {
    ...
    /* 0770, matching the app files directory this sits inside, NOT 0700.
     *
     * On a headset the save directory is already inside the app's own external
     * sandbox, so 0700 buys no privacy: the only things that can reach it are
     * this app and a cable. What it does buy is a directory the cable cannot
     * read, and the hub's Send to Quest backs the player's eeprom and VR
     * settings up over that cable before it replaces the app. A 0700 save
     * directory makes that backup silently find nothing, which is how a player
     * loses a save to an update and nobody sees an error. The parent is
     * drwxrws--- owned by the app with group ext_data_rw; this matches it. */
...
void *dkr_arena_init(uint32_t size) {
     ...
     * segment values (0x0N000000, segments 0-15 => 0x00000000..0x0FFFFFFF). On
     ...
     * the arena ABOVE the 256 MB segment ceiling so segment values always fall
...
#else
#if (defined(_ISOC11_SOURCE) || __STDC_VERSION__ >= 201112L) && !defined(__ANDROID__)
     ...
     * Android is excluded outright: bionic declares aligned_alloc only from API
     * 28, so on a lower target the header is silent and the call compiles to an
     * implicit declaration. posix_memalign below has been in bionic since the
     * start and allocates the same block, so nothing is lost by skipping the
     * C11 spelling there. */
...
u32 dkr_k0_to_physical(const void *x) {
     ...
     *   - already-truncated dkrptr32 values (value <= 0xFFFFFFFF): DKR frequently
 ...
 * pre-computed 32-bit values resolve through the segment table. Without the
...
/* MDKR_VR_PREWAIT=0 returns the present loop to sampling the sub-tick phase
 * BEFORE the headset wait (the pre-2026-08-19 order), for an in-headset A/B.
 * Read once. Flat runs never reach the caller (vr_frame_paced is false). */
static bool vr_prewait_enabled(void) {
    static int on = -1;
    if (on < 0) {
        const char *e = getenv("MDKR_VR_PREWAIT");
        on = (e != NULL && e[0] == '0') ? 0 : 1;
        fprintf(stderr, "[vr] present phase sampled %s the headset wait%s\n",
                on ? "AFTER" : "BEFORE",
                on ? "" : " (MDKR_VR_PREWAIT=0)");
        fflush(stderr);
    }
    return on != 0;
}
...
s32 osSendMesg(OSMesgQueue *mq, OSMesg msg, s32 flags) {
    ...
    if (mq && mq == s_schedInterruptQ) {
        ...
        /*
         * Ordinary libultra queues legitimately carry small integer values
...
s32 osRecvMesg(OSMesgQueue *mq, OSMesg *msg, s32 flags) {
    ...
    if (mq && mq == s_videoClientQueue) {
        if (s_viFieldsPending <= 0) {
            ...
            /*
             * VR stereo: re-draw the walked display list once per eye, HERE.
             *
             * This is the same instant the interpolated replay above uses, and
             * it has to be: the matrix registry the eye pass substitutes into
             * keys by Mtx pointer, and DKR builds the next list into that same
             * memory while this one is presented. Run from present instead and
             * every entry is a dead tenant -- measured as matrix hits=0, two
             * identical eyes, and a cross-eyed headset.
             *
             * AT ALPHA ZERO, not at the freshly-authored camera. DKR double-
             * buffers: the list the real walk just drew was AUTHORED ONE TICK
             * EARLIER (timeline proof, docs/architecture/presentation-
             * interpolation.md 1.1 step 1), so its own camera is the snapshot
             * pair's PREVIOUS entry -- exactly what the interpolated builder
             * returns at 0/1, and exactly the camera the flat mirror presents
             * for this same image. The old call substituted sVrAuthoredCameras,
             * which pass N's authoring has already overwritten with camera N:
             * a full-tick camera LEAD on every tick frame. Constant while eye
             * frames only happened at tick rate, so it read as nothing; the
             * moment correct-phase subloop frames interleaved with it, the
             * stream alternated +1.0-tick and +alpha-tick cameras every frame
             * -- a per-frame camera SAWTOOTH, felt as everything jittering and
             * as props wobbling against the world under head motion. The
             * authored-camera path remains the automatic fallback inside the
             * impl whenever no snapshot pair exists (level load, first tick).
             *
             * dl_fresh-GATED: DKR skips submitting a graphics task on some
             * ticks. On those, the retained list is a tick older than the
             * snapshot pair, so rendering eyes anyway pops the camera phase
             * for one frame, and a refusing walk turns the pair into a
             * one-frame PANEL flash. Submitting nothing lets the compositor
             * hold the last stereo frame, which is seamless.
             */
            if (dl_fresh) {
                platform_vr_render_eyes_interpolated(0u, 1u);
            }
            ...
            if (!oracle_variable_ticket && subloop && !catchup_ticket) {
                ...
                /* Hoisted out of the loop body: the VR pre-wait below needs
                 * the same "will this opportunity draw" answer the body uses. */
                extern int mdkr_vr_world_is_live(void);
                for (;;) {
                    /*
                     * UNDER A LIVE HEADSET, WAIT FIRST, THEN MEASURE.
                     *
                     * xrWaitFrame is the only pacer while VR is live, and
                     * until now it ran INSIDE the eye pass, after this
                     * iteration had already measured its wall time and fixed
                     * the sub-tick phase. So the phase was sampled wherever
                     * the previous frame's work happened to end, then the
                     * frame sat in the wait, then it was displayed on the
                     * compositor's grid: content timed to one clock, shown
                     * on another. the in-race trace (play_vr.24120.log,
                     * 8928 presents) has the signature: median 8.33 ms between
                     * samples but 20% under 7 ms and 15% over 10 ms -- the
                     * world advancing by uneven steps displayed at even ones,
                     * which is judder, loudest when the head turns. Waiting
                     * first puts the sample right after the wait, on the grid,
                     * for the flat walk, the eye pass and the tick detection
                     * alike (one clock per present); the eye pass's own
                     * vr_begin_frame then finds the frame begun and does not
                     * wait again. Only when this opportunity will actually
                     * draw -- a menu hold or a stale list presents nothing,
                     * and beginning a frame for nothing would leave it open
                     * until the next real present. The fake rig reads
                     * vr_frame_paced() false, so every headless number stays
                     * byte-identical (MDKR_VR_FAKE_PACED=1 opts it in for the
                     * measurement that proved this). MDKR_VR_PREWAIT=0 is the
                     * escape hatch back to the old order, for an A/B.
                     */
                    if (vr_frame_paced() && dl_fresh &&
                        !present_sched_render_elided() &&
                        present_sched_smoothing_enabled() &&
                        !(vr_is_active() && !mdkr_vr_world_is_live()) &&
                    ...
                    /* VR + menus/pause: stand the sub-tick re-walk down. The
                     * theater panel otherwise alternates between the real
                     * walk's menu image and this re-walk's at display rate,
                     * and any sub-pixel difference between the two shimmers
                     * -- reported as "the pause menu jitters". A steady
                     * tick-rate panel is the shipped, approved menu behavior.
                     * Flat runs (no VR) are untouched. */
                    const bool vr_menu_hold =
                        vr_is_active() && !mdkr_vr_world_is_live();
                    if (!present_sched_render_elided() && dl_fresh &&
                        ...
                        if (drew) {
                            ...
                            /* VR: the same sub-tick instant, per eye -- the
                             * display-rate stereo path. One stereo frame per
                             * headset refresh instead of one per 30 Hz tick;
                             * the compositor stops inventing three of every
                             * four frames, which is what props shearing
                             * under head motion and first-person jank were.
                             * Rides the same retained list and frozen
                             * registry the flat replay above just used, so
                             * the tenancy law that pinned the tick-rate eye
                             * pass to this file holds here too. Inert
                             * without a live session. */
                            platform_vr_render_eyes_interpolated(
