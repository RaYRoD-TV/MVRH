...
#include "vr/vr.h"               /* vr_is_active: stereo needs the same freeze */
...
bool present_sched_replay_armed(void) {
    ...
    /*
     * VR stereo re-walks the finished list once per eye, so it needs the same
     * frozen registry an interpolated present needs -- and it needs it whatever
     * the presentation-pace setting says, because stereo is not a smoothing
     * feature. Without this the freeze below never runs on a default build and
     * both eyes render black while every other signal looks healthy.
     */
    if (vr_is_active()) {
        return true;
    }
