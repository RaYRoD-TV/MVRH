...
/* The flat-window toast (engine_entry.h). Engine-side state so the VR layer
 * can feed it without touching the C++ shell. No clock here on purpose: this
 * file also builds into SDL-free test targets, so the OVERLAY owns the
 * timeout -- it notices the serial change and counts its own seconds. */
static char g_toastText[48];
static unsigned g_toastSerial = 0;

void platformOverlayVrModeToast(const char *name) {
    size_t i = 0;
    if (name == NULL || name[0] == '\0') {
        return;
    }
    for (; name[i] != '\0' && i + 1 < sizeof(g_toastText); i++) {
        g_toastText[i] = name[i];
    }
    g_toastText[i] = '\0';
    g_toastSerial++;
}

const char *platformOverlayVrModeToastText(void) {
    return g_toastSerial != 0 ? g_toastText : NULL;
}

unsigned platformOverlayVrModeToastSerial(void) {
    return g_toastSerial;
}
