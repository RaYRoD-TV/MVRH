...
/* Same choice gfx_opengl.c makes: on a GLES driver glad is not merely
 * unnecessary, it fails -- it parses GL_VERSION expecting "4.6" and a
 * headset answers "OpenGL ES 3.2 v1.r47". The ES entry points are linked
 * straight out of the NDK instead. */
...
#include "port_env.h"            /* MDKR_VR_THEATER, the stereo/theater A/B */
...
void mdkr_vr_startup_decide(int want_vr, int no_vr) {
#ifdef ENABLE_VR
    static int decided = 0;
    if (decided) return;
    decided = 1;
    if (no_vr || (!want_vr && !vr_headset_present())) {
        return; /* flat run: nothing changes, nothing is logged */
    }
    /* VR rides the GL backend (the OpenXR binding is WGL). Selecting it here,
     * before the first resolve, keeps mdkr_render_backend() the single source
     * of truth; if something already resolved another backend this run, VR is
     * refused honestly rather than binding a session to a context that does
     * not exist. Not an env write: on Windows SDL_setenv lands in the Win32
     * environment while getenv reads the CRT's snapshot, so an env round-trip
     * silently misses -- learned here, the hard way. */
    if (s_renderBackend >= 0 && s_renderBackend != MDKR_BACKEND_GL) {
        fprintf(stderr, "[vr] backend already resolved to %s; VR needs GL - "
                        "staying flat this run.\n", mdkr_render_backend_name());
        return;
    }
    s_renderBackend = MDKR_BACKEND_GL;
    vr_request_enable();
    /* THE DESK PAD KEEPS WORKING WITH THE HEADSET ON.
     *
     * SDL drops every joystick and controller transition while windows exist
     * and none of them is FOCUSED, and under a headset none ever is: the
     * runtime's own compositor holds focus, so the game window sits in the
     * background for the whole session. The pad then reads permanently
     * neutral -- no buttons, no D-pad -- which is exactly how the D-pad view
     * cycle looked like a dead binding rather than a dead event stream. The
     * app shell already discovered this for its hidden-window smoke test
     * (app_host.cpp says it in as many words) and lifted the rule for that
     * one path; VR needs the same lift for the real player.
     *
     * A hint CALLBACK inside SDL owns the flag, so setting it here -- after
     * SDL is up, at the moment VR is granted -- takes effect immediately and
     * needs no re-init. Flat runs never reach this line, so a windowed
     * desktop session keeps SDL's shipped focus behaviour. */
    SDL_SetHintWithPriority(SDL_HINT_JOYSTICK_ALLOW_BACKGROUND_EVENTS, "1",
                            SDL_HINT_OVERRIDE);
    {
        /* Read back rather than assumed: a hint that failed to take is a dead
         * gamepad in the headset, and that is not something to discover from
         * a player's report. */
        const char *live = SDL_GetHint(SDL_HINT_JOYSTICK_ALLOW_BACKGROUND_EVENTS);
        fprintf(stderr, "[vr] desk gamepad while unfocused: %s\n",
                (live != NULL && live[0] == '1') ? "on" : "OFF");
    }
    /* The build stamps itself so a log always names the exe it came from:
     * two rounds of in-headset verdicts were nearly spent against a stale
     * binary held open by an earlier session. */
    fprintf(stderr, "[vr] %s - GL backend selected for OpenXR (WGL). "
            "[build %s %s]\n",
            want_vr ? "--vr" : "headset detected", __DATE__, __TIME__);
    /* Model detail held out to the far ladder rungs: the authored N64 bands
     * drop racers to silhouette cones at distances a 240p screen hid and a
     * sharp eye buffer shows plainly. CPU-side, costs no memory.
     *
     * The eye RENDER SCALE is deliberately NOT set here: it is derived from
     * the headset's own eye size once the session is up (see
     * vr_apply_eye_render_scale). A literal here has no idea how large the
     * window is, and a 4x literal against a desktop-sized window is a 16x
     * pixel allocation the GL driver refuses -- measured as an access
     * violation inside nvoglv64 on the first draw, four launches running. */
    mdkr_video_config_vr_floor(MDKR_ENH_LOD_BIAS, 2.0f);
    /* Draw distance to the schema's ceiling: every distance cull is authored
     * against the GAME camera and a 240p screen, and a headset eye parked
     * near the wedge's radius watches props pop at a fixed ring. The whole
     * enhancement is draw-only by construction (the authoritative tick keeps
     * the authored radius), so the simulation cannot tell. A player's own
     * pin still wins - this is a floor, not an order. */
    mdkr_video_config_vr_floor(MDKR_ENH_DRAW_DISTANCE, 400.0f);
    /* Presentation pace Smooth, the VR baseline: FrameLimit "display" arms
     * the host-opportunity subloop and MotionSmoothing "interpolate" arms
     * the retained-list replay the display-rate eye pass rides. Without
     * them the headset gets ONE stereo frame per 30 Hz game tick and the
     * runtime warps the other three of every four frames -- felt as props
     * and textures shearing with head motion, and as first-person jank.
     * Live-only, never persisted, yields to the player's env/CLI pins. */
    mdkr_video_config_vr_set_live_text(MDKR_VIDEO_FRAME_LIMIT, "display");
    mdkr_video_config_vr_set_live_text(MDKR_VIDEO_MOTION_SMOOTHING,
                                       "interpolate");
    /* The presentation snapshot feeds the billboard owner registrations the
     * eye walk rebuilds WORLD sprite planes from; without it the packet is
     * empty and sprites keep their authored screen-glued matrices (measured
     * as reg=0, every substitution refused). Env-dormant by default; VR is
     * a standing consumer. */
    {
        extern void presentation_snapshot_set_enabled(bool enabled);
 ...
 * unchanged. Blank lines and `#` comments ignored. Directional values drive the
...
static void sdl_gl_backpressure_after_swap(void) {
    ...
    /* This test asks whether the LOADER found the sync entry points, so it only
     * means anything where they are glad's function pointers. On the GLES arm
     * they are ordinary linked functions -- sync objects are core from ES 3.0 --
     * and taking their address can never be null, which the compiler correctly
     * calls a tautology. */
...
static void sdl_apply_gl_present_policy(void) {
    ...
    /* While a VR session lives, xrWaitFrame is the ONLY pacer: a vsynced
     * desktop mirror fights it and locks the app near half the monitor
     * rate (measured live as ~28 fps on a 120 Hz headset, ~100%%
     * reprojected -- felt as the whole world shearing under head motion).
     * The session-READY handler clears the interval once; this covers
     * every later re-apply (video config, window events) for as long as
     * the session is up. */
    const int requested =
        (automation || vr_is_active()) ? 0 : (tearing ? -1 : 1);
    int result;
    int effective;
...
#ifdef MGB64_PORTMASTER_GLES
    /* ES 3.2 is what an Adreno in a Quest 3 offers and what the renderer's GLES
     * arm is written against. The OpenXR session binds to THIS context, so the
     * profile asked for here is the profile the runtime has to be able to share
     * its swapchain textures with. */
...
/*
 * Hold-to-look-behind. Not an N64 button (the game has none free: a real L
 * press fires thread3's internal level-transition machinery), so it is a
 * port-side held flag fed by the keyboard (C), the VR controllers (Y, and
 * the first-person L3 hold), and an unbound gamepad L3, consumed by
 * cam_build_view_basis (game/src/camera.c) as a pure view rotation.
 */
static int s_lookBehindKb;
static int s_lookBehindVr;
static int s_lookBehindPad;
/* Set while the physical right trigger is held; read (and cleared) by the
 * pause menu's options shortcut. See the note at the assignment. */
static int s_optionsShortcutPad;
...
static void gc_read(
    ...
    /*
     * THE OPTIONS SHORTCUT IS THE PHYSICAL RIGHT TRIGGER, not an N64 button.
     * The pause menu used to open the page on N64 R_TRIG, which meant the
     * shortcut moved whenever R was rebound: with the old pad layout it sat
     * on the right SHOULDER, which is exactly what got reported. Reading the
     * trigger itself pins the shortcut to the control it is named after, and
     * leaves the player free to bind R wherever they like.
     */
    s_optionsShortcutPad =
        normalized.active[MDKR_CONTROLLER_SOURCE_RIGHT_TRIGGER] ? 1 : 0;
    ...
    /* A gamepad's L3 doubles as look-behind while its binding stays "none"
     * (the shipped default). A player who bound the click to an N64 action
     * made a choice, and the glance then stays on keyboard C / VR Y. */
    if (normalized.active[MDKR_CONTROLLER_SOURCE_LEFT_STICK] &&
        strcmp(mdkr_video_config_current()
                   ->values[MDKR_INPUT_CONTROLLER_LEFT_STICK].text,
               "none") == 0) {
        s_lookBehindPad = 1;
    }
...
/*
 * True on the FRAME THE TRIGGER GOES DOWN, not while it is held. The flag it
 * reads tracks the live trigger (assigned every pad read, not latched):
 * a latch would still be set from the last drift of the race, and pausing
 * would then drop the player straight into the options page.
 */
int mdkr_options_shortcut_pressed(void) {
    static int wasDown;
    const int down = s_optionsShortcutPad;
    const int edge = down && !wasDown;
    wasDown = down;
    return edge;
}

int mdkr_look_behind_held(void) {
    /* MDKR_TEST_LOOK_BEHIND=1 holds the flag for headless image A/Bs -- the
     * keyboard is unreachable from an --input-script run. */
    static int forced = -1;
    if (forced < 0) {
        const char *v = getenv("MDKR_TEST_LOOK_BEHIND");
        forced = (v != NULL && v[0] != '\0' && v[0] != '0') ? 1 : 0;
    }
    return forced || s_lookBehindKb || s_lookBehindVr || s_lookBehindPad;
}
...
static void kbd_read(struct pad_state *p) {
     ...
     *   I J K L        C-buttons      camera zoom / course directions
     *   C (hold)       look behind    port feature; the N64 game has no rear view
     *   Enter          Start
     *
    ...
    /* Hold to look behind -- not an N64 button; the view builder reads it. */
    s_lookBehindKb = k[SDL_SCANCODE_C] ? 1 : 0;
...
/* The same answer, for game code that must keep the AUTHORED flow under a
 * scripted fixture while changing it for real players (menu.c's caution
 * card). Exported rather than duplicated so there is one definition of "a
 * test is driving this run". */
int mdkr_test_script_driving(void) {
    return test_script_only_input();
}
...
static void input_capture_live(uint64_t target_tick) {
    ...
    /* Re-derived from the pads below every capture; a disconnected or
     * suppressed pad must never leave a rear glance latched on. */
    s_lookBehindPad = 0;
    for (port = 0; port < DKR_MAXPADS; port++) {
...
static void platform_surface_visibility_update(void) {
    ...
    /* A minimized or hidden desktop window is not the display that matters
     * under a headset: eliding the walk there would stop the eye pass too
     * (and with it the whole headset image), so a live VR session never
     * elides on surface visibility. */
...
void platform_input_commit_tick(uint64_t ticket) {
    ...
    /* D-pad UP shifts the view mode. The RAW SDL button, deliberately not
     * the merged N64 bit: keyboard up and stick deflection mirror onto
     * N64_DU, and steering must never cycle the view. Once per tick (this is
     * the one per-tick commit), press edge only, and never from a scripted
     * run -- the harness stays deterministic.
     *
     * NOT gated on a live headset: third person, first person and diorama
     * all move the camera on a monitor now (camera.c), so the binding
     * belongs to the game rather than to the session. In a build with no VR
     * compiled in the cycle is an inline no-op, so this costs one compare. */
    /* MDKR_VR_VIEWMODE pins the opening view on a flat run the same way it
     * does for a headset run: how these views get verified without a hand on
     * a pad, and a fair knob for anyone who always plays one way. OUTSIDE
     * the input gate below on purpose -- a scripted run sets exactly that
     * gate, so a pin inside it could never reach the verification runs it
     * exists for. It is an env choice, not an input, and it is applied once. */
    if (!vr_is_active()) {
        extern void mdkr_vr_view_knobs_first_use(void);
        mdkr_vr_view_knobs_first_use();
    }
    {
        /* Defined below with the other VR-knob helpers; declared here rather
         * than in a header because this is the only caller. */
        extern void mdkr_vr_quality_governor(void);
        mdkr_vr_quality_governor();
    }
    if (!test_script_only_input() && !s_gameInputSuppressed) {
        /* The cycle fires only while the WORLD is what the player is
         * driving: never in frontend menus, never while the pause or
         * finish menu is up. Paused, the D-pad is menu navigation --
         * stepping the highlight AND spinning the view behind the menu
         * read as two hands on one wheel. Both answers are the game's own
         * (thread3_main.c), so the platform never guesses at menu state. */
        extern int mdkr_vr_world_is_live(void);
        extern int mdkr_vr_menu_is_up(void);
        static Uint8 sPrevDpadUp;
        const Uint8 up =
            s_controllerSource[0].buttons[SDL_CONTROLLER_BUTTON_DPAD_UP];
        if (up && !sPrevDpadUp && mdkr_vr_world_is_live() &&
            !mdkr_vr_menu_is_up()) {
            vr_cycle_view_mode();
        }
        sPrevDpadUp = up;
...
#endif
        /* VR session pump, GL path only (the OpenXR session is bound to this
         * thread's WGL context). Inert until VR was requested at startup;
         * with a session live it polls session events and runs one
         * xrWaitFrame/xrBeginFrame + xrEndFrame pair per presented frame.
         * Between the pair, the finished back buffer (scene + HUD + overlay,
         * exactly what the swap is about to show) is fed to the theater
         * panel; pre-swap, because the back buffer is undefined after. When
         * eye rendering lands, vr_begin_frame moves to the frame's START so
         * xrWaitFrame paces the render, not the swap tail. */
        vr_begin_frame();
        {
            /* The eye pass already ran, back when the display list was walked
             * (platform_vr_render_eyes). Theater is the fallback, not a second
             * copy of the world: the panel submit refuses on any frame that
             * rendered eyes. */
            int vrFbW = 0, vrFbH = 0;
            platform_sdl_drawable_size(&vrFbW, &vrFbH);
            vr_panel_feed_backbuffer(vrFbW, vrFbH);
        }
        sdl_gl_resource_heartbeat("before-swap", 0);
        /*
         * THE DESKTOP MIRROR IS NOT THE DISPLAY THAT MATTERS, and under a
         * headset it was costing more than it was worth.
         *
         * Every present swapped the window at the headset's rate -- 120 a
         * second -- with the swap interval forced to 0, because a vsync'd
         * swap would block this thread and a blocked thread misses the
         * headset's deadline. Presenting without vsync IS tearing: that is
         * the whole mechanism, and at 120 swaps a second the tear is
         * constantly visible on the monitor.
         *
         * A headset session does not need the monitor at the headset's rate.
         * The mirror now swaps about thirty times a second, so the tear it
         * can produce becomes rare instead of continuous, and three of every
         * four presents skip the swap entirely -- time handed straight back
         * to the frame budget that was reprojecting. MDKR_VR_MIRROR=off
         * stops the swap altogether, which is the only way to be certain of
         * ZERO tearing while the runtime owns the pacing.
         *
         * The theater panel is unaffected: vr_panel_feed_backbuffer above
         * reads the BACK buffer, before any swap, so the panel keeps every
         * frame whether the desktop window is updated or not.
         */
        {
            int mirrorSwap = 1;
#ifdef ENABLE_VR
            if (vr_is_active()) {
                static int mirrorOff = -1;
                static Uint32 mirrorPeriodMs = 33u;
                static Uint32 mirrorNextAt = 0;
                const Uint32 now = SDL_GetTicks();
                if (mirrorOff < 0) {
                    /* One reading shared with the eye blit (vr_openxr.cpp):
                     * "0"/"off"/"no" = off; a bare 'o' used to count and
                     * made "on" mean off. MDKR_VR_MIRROR_HZ=<n> sets the
                     * swap cadence (default 30): the window now shows the
                     * headset eye, so head motion on the monitor steps at
                     * this rate. Each swap costs the headset loop real time
                     * on the GL thread, which is why 30 is the default --
                     * raise it and watch the "reprojected" figure. */
                    const char *hz = getenv("MDKR_VR_MIRROR_HZ");
                    int rate = (hz != NULL && hz[0] != '\0') ? atoi(hz) : 30;
                    if (rate < 1) rate = 1;
                    if (rate > 240) rate = 240;
                    mirrorPeriodMs = (Uint32)(1000 / rate);
                    mirrorOff = vr_mirror_env_is_off() ? 1 : 0;
                    fprintf(stderr, "[vr] desktop mirror: %s%d a second%s\n",
                            mirrorOff ? "off (no swap, no tearing; would be "
                                      : "about ",
                            rate, mirrorOff ? ")" : "");
                }
                if (mirrorOff) {
                    mirrorSwap = 0;
                } else if (now < mirrorNextAt) {
                    mirrorSwap = 0;
                } else {
                    mirrorNextAt = now + mirrorPeriodMs;
                }
            }
#endif
            if (mirrorSwap) {
...
#endif
        /* A live OpenXR session paces this loop itself: xrWaitFrame inside
         * vr_begin_frame blocks at the headset refresh, and the donor law is
         * that while VR is live it is the ONLY pacer -- a second grid here
         * beats against it and delivers a cadence below either rate. Elapsed
         * time is still measured and credited below; only the sleeps stand
         * down. The fake rig reports false, so headless pacing (and every
         * deterministic harness number) is untouched. */
        if (vr_frame_paced()) {
            deadline = false;
            clock = NULL;
        }
...
int platform_pad_rumble_supported(int port) {
    if (port == 0 && vr_controllers_active()) return 1;
    if (port < 0 || port >= DKR_MAXPADS) return 0;
#ifdef __EMSCRIPTEN__
...
int platform_pad_rumble(int port, int enabled) {
    int audible;
    const int vrHands = port == 0 && vr_controllers_active();
    if (port < 0 || port >= DKR_MAXPADS ||
        (s_gc[port] == NULL && !vrHands
#ifdef __EMSCRIPTEN__
    ...
    /*
     * THE SAME MOTOR, FELT IN THE HANDS. Every Rumble Pak start and stop the
     * game issues -- wall impacts, wrecks, zippers, brake judder -- now
     * reaches the VR controllers, not only a gamepad. Until this, the whole
     * pak system produced exactly one haptic in VR: a menu confirmation
     * blip, so contact with walls and racers was silent in the player's
     * hands. Same contract as the SDL path below: a long buzz with an
     * explicit stop (the pak is a binary motor the game itself pulses), the
     * same strength profile, and stops always delivered.
     */
    if (vrHands) {
        if (audible) {
            const float f = (float)mdkr_controller_rumble_output_strength(
                                mdkr_video_config_current(), 1) / 65535.0f;
            vr_controller_rumble(f, 60.0f);
        } else {
            vr_controller_rumble_stop();
        }
    }
    if (s_gc[port] == NULL) {
        return 1;   /* the VR hands took it; there is no pad to also buzz */
    }
...
/*
 * VR stereo: draw the finished frame again, once per eye.
 *
 * DKR builds ONE display list per graphics task. This port already owns the
 * machinery to re-walk that list with a substituted per-camera
 * view-projection -- it is how motion smoothing presents an interpolated
 * image -- and a stereo eye is the same operation with a different matrix, so
 * stereo re-uses it rather than adding a second way to draw the world. The
 * walk never re-enters game code and never touches tick state.
 *
 * Order matters here: this runs after the game finished drawing (the flat
 * frame is already in the back buffer for the desktop mirror) and inside the
 * XR frame opened by vr_begin_frame, so the eye images and the compositor
 * submit belong to the same predicted display time.
 *
 * MDKR_VR_THEATER=1 forces the theater panel instead -- the A/B that tells a
 * stereo defect apart from a defect the flat frame has too.
 */
#ifdef ENABLE_VR
/* game/src/camera.c (NATIVE_PORT): per-eye view-projections built from the
 * authored camera and the headset's full eye view (head pose included). */
extern size_t mdkr_camera_vr_view_projections(
    const float eye_view_units[16],
    float angle_left, float angle_right, float angle_up, float angle_down,
    GfxShadowReplayViewProjection *out, size_t capacity);
/* The sub-tick sibling: the same compose from the INTERPOLATED camera pair,
 * horizon flatten re-applied (the snapshot stores the raw rotation). Returns
 * 0 when no interpolable pair exists; the authored builder is the fallback. */
extern size_t mdkr_camera_vr_view_projections_interpolated(
    const float eye_view_units[16],
    float angle_left, float angle_right, float angle_up, float angle_down,
    uint64_t numerator, uint64_t denominator,
    GfxShadowReplayViewProjection *out, size_t capacity);
/* Sprite counter-roll computed beside the eye VPs: the on-screen lean of
 * world-up through the primary camera's composed view. */
extern float mdkr_camera_vr_billboard_roll(void);

/*
 * The eye render resolution, derived from the DEVICE.
 *
 * The display list rasters at the game's render resolution (output x
 * Video.RenderScale) and its viewport/scissor commands carry those
 * coordinates, so the eye target must match it -- which makes
 * Video.RenderScale the only knob that sharpens the eyes. The ceiling is
 * therefore not a preference and not a literal: rendering more pixels than
 * the headset's own eye swapchain can show is pure waste (the eye blit
 * downscales them), and asking for them is what the GL driver refused with
 * an access violation on the first draw when a 4x literal met a
 * desktop-sized window (a 16x pixel allocation, MSAA on top).
 *
 * So: never more pixels per eye than the eye swapchain has, window aspect
 * preserved, clamped to the schema's 1..4. Re-evaluated every frame and
 * SET in either direction, so a window resize brings it down instead of
 * leaving a stale multiplier behind. MSAA stands down once the scale is
 * doing the anti-aliasing itself (the schema's own words: "Redundant with
 * render scale") -- it is a 4x memory multiplier on the largest targets
 * in the frame and buys nothing at these resolutions.
 */
static void vr_apply_eye_render_scale(void) {
    const int eyeW = vr_eye_width(0);
    const int eyeH = vr_eye_height(0);
    int outW = 0, outH = 0;
    double budget, have, scale;

    if (eyeW <= 0 || eyeH <= 0) {
        return;                     /* session not up yet: nothing to derive */
    }
    platform_sdl_drawable_size(&outW, &outH);
    if (outW <= 0 || outH <= 0) {
        return;
    }
    budget = (double)eyeW * (double)eyeH;
    have = (double)outW * (double)outH;
    scale = sqrt(budget / have);
    if (scale < 1.0) {
        scale = 1.0;                /* never render BELOW the window */
    }
    if (scale > 4.0) {
        scale = 4.0;                /* schema ceiling */
    }
    /* Quarter steps: a hair of extra sharpness is not worth rebuilding
     * every render target because the window moved by a pixel. */
    scale = floor(scale * 4.0 + 0.5) / 4.0;
    mdkr_video_config_vr_set_live(MDKR_VIDEO_RENDER_SCALE, (float)scale);
    {
        static int msaaStoodDown = -1;
        const int want = (scale >= 2.0) ? 0 : 4;
        if (msaaStoodDown != want) {
            msaaStoodDown = want;
            vr_set_msaa(want);
            fprintf(stderr,
                    "[vr] eye render %dx%d (scale %.2f of %dx%d, headset eye "
                    "%dx%d), MSAA %s\n",
                    (int)(outW * scale), (int)(outH * scale), scale, outW,
                    outH, eyeW, eyeH,
                    want ? "4x" : "off (render scale supersamples)");
            fflush(stderr);
        }
    }
}

/* Set by the finish-dump hook: the next HUD pass also writes vr_hud.tga. */
static int s_vrHudDumpAtFinish = 0;

static void vr_render_stereo_eyes(int frameW, int frameH,
                                  uint64_t alphaNum, uint64_t alphaDen,
                                  int interpolated) {
    static int stereoOptIn = -1;
    static int loggedEyes = 0;
    int eye;
    int eyesRendered = 0;

    /*
     * STEREO IS THE DEFAULT, because it now PASSES its measurements
     * (2026-08-12): substitution ground-truthed against the flat frame,
     * vertical disparity ~0, one consistent sign, magnitude p50 13' at
     * 32 units/m, disparity monotonic with distance and linear in the scale
     * knob -- and the eye view carries the FULL head pose (rotation proven
     * by the MDKR_VR_FAKE_PITCH harness), so the world holds still while
     * the head moves. MDKR_VR_STEREO=0 falls back to the theater panel.
     */
    if (stereoOptIn < 0) {
        stereoOptIn = port_env_bool("MDKR_VR_STEREO", 1,
                                    "VR: stereo eye pass "
                                    "(0 falls back to the theater panel)");
        if (!stereoOptIn) {
            fprintf(stderr, "[vr] MDKR_VR_STEREO=0: theater panel only\n");
            fflush(stderr);
        }
    }
    if (!stereoOptIn || !vr_is_active() || frameW <= 0 || frameH <= 0) {
        return;
    }
    /* Menus and pause are 2D compositions: skipping the eye pass here lets
     * vr_submit fall back to the world-anchored theater panel, which is the
     * one surface a 2D screen fuses on. Gameplay resumes stereo on the next
     * frame -- a cut on the mode edge, per the house law. */
    {
        extern int mdkr_vr_world_is_live(void);
        if (!mdkr_vr_world_is_live()) {
            return;
        }
    }

    /* Render each eye at the game's own frame size so the display list's
     * viewport fills the target; vr_end_eye stretches it into the eye
     * swapchain, which is exact because NDC carries no aspect. */
    vr_set_eye_raster_override(frameW, frameH);

    /* MDKR_VR_PERF=1: per-stage wall clocks for the stereo frame, printed
     * every 300 stereo frames. The pacing line's "world" bucket only says
     * the eye walks own the frame; this says which STAGE of them does. */
    static int perfOn = -1;
    static double perfBegin, perfWalk, perfEnd, perfHud;
    static unsigned perfFrames;
    if (perfOn < 0) {
        const char *env = getenv("MDKR_VR_PERF");
        perfOn = (env != NULL && env[0] != '\0' && env[0] != '0') ? 1 : 0;
    }

    for (eye = 0; eye < 2; eye++) {
        GfxShadowReplayViewProjection views[4];
        float offsetMetres[3];
        float eyeViewUnits[16];
        float angleL = 0.0f, angleR = 0.0f, angleU = 0.0f, angleD = 0.0f;
        float unitsPerMetre;
        size_t count;
        Uint64 t0 = 0, t1 = 0, t2 = 0, t3 = 0;

        if (!vr_eye_view_params(eye, offsetMetres, &angleL, &angleR,
                                &angleU, &angleD)) {
            break;      /* views not located this frame: no half pairs */
        }
        /* The COMPOSED eye view (head rotation + IPD + comfort clamps,
         * recenter-relative, already in game units) -- this is what carries
         * head TRACKING into the eyes, not just the eye separation. */
        if (!vr_eye_view_units(eye, eyeViewUnits)) {
            break;
        }
        if (perfOn) {
        ...
        unitsPerMetre = vr_units_per_meter();
        /* Sub-tick frames compose from the interpolated camera pair; the
         * authored builder is the fallback whenever no pair exists (first
         * tick after a load, teleports), so stereo never drops to the
         * panel mid-race. haveInterp also picks the WALK below: authored
         * views must ride the authored (alpha-0) walk or the world and the
         * objects would sit at different instants. */
        int haveInterp = 0;
        count = 0;
        if (interpolated) {
            count = mdkr_camera_vr_view_projections_interpolated(
                eyeViewUnits, angleL, angleR, angleU, angleD,
                alphaNum, alphaDen,
                views, sizeof(views) / sizeof(views[0]));
            haveInterp = count > 0;
        }
        if (!haveInterp) {
            count = mdkr_camera_vr_view_projections(
                eyeViewUnits, angleL, angleR, angleU, angleD,
                views, sizeof(views) / sizeof(views[0]));
        }
        /* Billboard sprites carry clip-space (screen) bases; hand the walk
         * world-up's NDC direction through this eye's composed view so
         * flora stays world-upright while the head moves. Computed beside
         * the eye VPs because it needs the AUTHORED view and the eye
         * projection; the walk turns it into a PIXEL-space rotation (a raw
         * clip rotation shears -- stretched, swaying sprites). */
        {
            extern void mdkr_camera_vr_billboard_axes(float out[16]);
            float f[16];
            mdkr_camera_vr_billboard_axes(f);
            gfx_dkr_set_eye_billboard_axes(f);
        }
        int eyeDrew;
        {
            /* A refusal and an empty camera set are different defects and
             * both look like a black eye, so the first frame says which. */
            /* Alpha 0 is an ENDPOINT, not an interior image: the tick-block
             * eye pass runs the endpoint walk (object substitution at 0/1
             * with the endpoint exemption). The interpolated walk at 0/1
             * would be interior-strict and refuse across captures the
             * endpoint is entitled to, and every such refusal would fall
             * back to the authored (next-tick) camera -- the exact sawtooth
             * frame this phase work removes. */
            int drew = (count > 0)
                ? ((haveInterp
                        ? (alphaNum == 0u
                               ? gfx_dkr_replay_walk_endpoint(views, count)
                               : gfx_dkr_replay_walk_interpolated(
                                     views, count, alphaNum, alphaDen))
                        : gfx_dkr_replay_walk(views, count))
                       ? 1 : 0)
                : -1;
            /* The interpolated walk refuses across transitions (list swap,
             * level load, registry not yet frozen). A refused eye is a
             * cleared target, and submitting it is a BLACK FLASH at headset
             * rate.
             *
             * AUTHORED FALLBACK BY DEFAULT -- the cost, not the phase,
             * decides. The phase-correct fallback (endpoint walk at alpha 0,
             * then the authored builder as a last resort) is the better
             * image, but it pays up to two EXTRA full display-list walks per
             * eye on exactly the frames the machine is already late on:
             * refusals cluster where frames run long, so the repair bill
             * lands on the struggling frames and compounds. Shipped that way
             * (patch 6) it came back from the field as "stuttery crackly
             * mess", and the desk A/B agrees: paced fake-rig race, 210 s,
             * audio silence 22-26k frames and 5-7 underruns with the phased
             * fallback against 6-8k and 3 with the authored jump -- which is
             * the field-proven patch-5 behavior. So the cheap single
             * authored walk (one tick of camera lead on a refused present,
             * the 08-16 sawtooth note) is the default again, and
             * MDKR_VR_REFUSE_AUTHORED=0 opts into the phased fallback for
             * the round that makes refusal predictable BEFORE the first
             * walk, where phase-correctness costs nothing extra. */
            if (drew == 0 && haveInterp) {
                static int refuseAuthored = -1;
                if (refuseAuthored < 0) {
                    const char *e = getenv("MDKR_VR_REFUSE_AUTHORED");
                    refuseAuthored = (e != NULL && e[0] == '0') ? 0 : 1;
                }
                if (!refuseAuthored) {
                    count = mdkr_camera_vr_view_projections_interpolated(
                        eyeViewUnits, angleL, angleR, angleU, angleD,
                        0u, 1u, views, sizeof(views) / sizeof(views[0]));
                    drew = (count > 0)
                        ? (gfx_dkr_replay_walk_endpoint(views, count) ? 1 : 0)
                        : 0;
                }
                if (drew <= 0) {
                    count = mdkr_camera_vr_view_projections(
                        ...
                        views, sizeof(views) / sizeof(views[0]));
                    drew = (count > 0)
                        ? (gfx_dkr_replay_walk(views, count) ? 1 : 0)
                        : -1;
                }
                haveInterp = 0;
            }
            /* Periodic, not once: the first eye pass happens on a title-sized
             * display list, and a single early sample cannot say whether
             * substitution works in a real scene. A late reading with hits
             * still at zero is the real verdict. */
            static unsigned vrEyeLogTick = 0u;
            if (eye == 0) {
                vrEyeLogTick++;
            }
            if (!loggedEyes || (vrEyeLogTick % 300u) == 0u) {
                /* hits vs misses vs rejects says whether the eye matrix was
                 * actually SUBSTITUTED. Identical eyes with a healthy "drew"
                 * mean the walk used the game's own matrices. */
                extern size_t gfx_shadow_replay_frozen_matrix_count(void);
                ...
                uint64_t walks = 0, hits = 0, misses = 0, rejects = 0, real = 0;
                uint64_t bbHits = 0, bbHolds = 0, bbVh = 0, bbVo = 0;
                gfx_dkr_replay_get_stats(&walks, &hits, &misses, &rejects, &real);
                gfx_dkr_replay_get_billboard_stats(&bbHits, &bbHolds, &bbVh,
                                                   &bbVo);
                fprintf(stderr,
                        "[vr] eye %d: cameras=%u camera_id=%d bbroll=%.1fdeg "
                        "replay=%s alpha=%llu/%llu%s "
                        "lock=%d mode=%d bbcard=%llu bbfallback=%llu "
                        "(matrix hits=%llu misses=%llu rejects=%llu "
                        "bbworld=%llu frozen=%zu)\n", eye,
                        (unsigned)count, count ? views[0].camera_id : -1,
                        (double)(mdkr_camera_vr_billboard_roll()
                                 * 57.29577951f),
                        drew < 0 ? "SKIPPED (no authored camera)"
                                 : (drew ? "drew" : "REFUSED"),
                        (unsigned long long)(haveInterp ? alphaNum : 0u),
                        (unsigned long long)(haveInterp ? alphaDen : 1u),
                        haveInterp ? " (interp)"
                                   : (interpolated ? " (authored fallback)"
                                                   : ""),
                        vr_get_horizon_lock() ? 1 : 0,
                        vr_get_view_mode(),
                        (unsigned long long)
                            gfx_dkr_eye_card_vertex_count(),
                        (unsigned long long)
                            gfx_dkr_eye_billboard_rolled_count(),
                        (unsigned long long)hits,
                        (unsigned long long)misses,
                        (unsigned long long)rejects,
                        (unsigned long long)bbHits,
                        gfx_shadow_replay_frozen_matrix_count());
                fflush(stderr);
            }
            eyeDrew = drew;
        }
        ...
        /* A pass that drew nothing (both walks refused: the walk-entry state
         * is not ready on the very first frame after a level load) must not
         * become a submitted eye -- a cleared target composited at headset
         * rate is a black flash. Restore the framebuffer state and stop; with
         * fewer than two eyes submitted the frame falls back to the panel,
         * which carries the flat frame the player expects during a load. */
        if (eyeDrew <= 0) {
            vr_end_eye(eye);
            break;
        }
        ...
        if (perfOn) {
            ...
            perfBegin += (double)(t1 - t0) / f;
            perfWalk += (double)(t2 - t1) / f;
            perfEnd += (double)(t3 - t2) / f;
            if (eye == 1) {
                perfFrames++;
                if (perfFrames == 300u) {
                    fprintf(stderr,
                            "[vrperf] per stereo frame (300-frame mean): "
                            "begin %.2f ms, walks %.2f ms, end %.2f ms, "
                            "hud %.2f ms\n",
                            1000.0 * perfBegin / 300.0,
                            1000.0 * perfWalk / 300.0,
                            1000.0 * perfEnd / 300.0,
                            1000.0 * perfHud / 300.0);
                    fflush(stderr);
                    perfBegin = perfWalk = perfEnd = perfHud = 0.0;
                    perfFrames = 0;
                }
            }
        }
        if (!loggedEyes && eye == 1) {
            fprintf(stderr,
                    "[vr] stereo eyes live (%dx%d per eye, %.1f units/metre, "
                    "eye offset %.4f m)\n",
                    frameW, frameH, (double)unitsPerMetre,
                    (double)offsetMetres[0]);
            fflush(stderr);
            loggedEyes = 1;
        }
    }
    gfx_dkr_set_eye_billboard_roll(0.0f);
    gfx_dkr_set_eye_billboard_axes(NULL);

    /*
     * The HUD, on its own plane: one more walk of the same list drawing ONLY
     * the 2D, into a transparent texture the VR layer parks on the HUD quad
     * (donor size/distance/anchor laws apply). Flat in-eye 2D cannot fuse
     * and rides every head motion; a quad at a real distance does neither.
     */
    {
        /* The HUD plane is ON by default: the 2D-only walk lands the race
         * HUD on real transparency (measured: position, lap, bananas, timer,
         * GO! mid-fade and the minimap over alpha 0 -- the empty-texture
         * blocker was the alpha floor turning blended 2D opaque, fixed in
         * gfx_opengl_set_uniforms). MDKR_VR_HUD=0 is the escape hatch back
         * to no HUD in stereo. */
        static int hudPlaneOptIn = -1;
        if (hudPlaneOptIn < 0) {
            const char *env = getenv("MDKR_VR_HUD");
            hudPlaneOptIn =
                (env != NULL && env[0] != '\0' && env[0] == '0') ? 0 : 1;
        }
        if (!hudPlaneOptIn) {
            return;
        }
    ...
    if (eyesRendered == 2) {
        extern unsigned int gfx_opengl_vr_hud_texture(int *w, int *h);
        extern bool gfx_opengl_vr_hud_dump(const char *path);
        const Uint64 th0 = perfOn ? SDL_GetPerformanceCounter() : 0;
        const bool hudWalked = gfx_dkr_replay_walk_vr_hud();
        if (perfOn) {
            perfHud += (double)(SDL_GetPerformanceCounter() - th0) /
                       (double)SDL_GetPerformanceFrequency();
        }
        if (hudWalked) {
            int hw = 0, hh = 0;
            unsigned int tex = gfx_opengl_vr_hud_texture(&hw, &hh);
            if (tex != 0u && hw > 0 && hh > 0) {
                /* In-race pause: stereo stays live (the world freezes
                 * behind) and the 2D composite -- pause dialogs, the VR
                 * OPTIONS page -- rides this same HUD texture. Menu
                 * anchoring gives the quad its fixed readable width at the
                 * menu distance, and every VR knob is visible in the world
                 * as it moves. */
                extern int mdkr_vr_menu_is_up(void);
                extern int mdkr_vr_menu_hud_preview(void);
                /* WYSIWYG for the HUD rows: while one is highlighted the
                 * quad drops to its REAL HUD size and distance, so the
                 * thing being adjusted is the thing on screen. Everywhere
                 * else the paused composite keeps MENU anchoring. */
                vr_set_hud_menu(mdkr_vr_menu_is_up() != 0 &&
                                mdkr_vr_menu_hud_preview() == 0);
                (void)vr_submit_hud_texture(tex, hw, hh);
                /* MDKR_VR_HUDDUMP=<n>: write the nth HUD texture to
                 * vr_hud.tga -- the headless proof the pass carries the HUD
                 * on transparency. */
                {
                    static long hudDumpFrame = -2;
                    static long hudPasses = 0;
                    if (hudDumpFrame == -2) {
                        const char *env = getenv("MDKR_VR_HUDDUMP");
                        hudDumpFrame =
                            (env && env[0]) ? atol(env) : -1;
                    }
                    hudPasses++;
                    if (s_vrHudDumpAtFinish) {
                        s_vrHudDumpAtFinish = 0;
                        hudDumpFrame = hudPasses + 30;
                    }
                    if (hudDumpFrame >= 0 && hudPasses == hudDumpFrame) {
                        if (gfx_opengl_vr_hud_dump("vr_hud.tga")) {
                            fprintf(stderr,
                                    "[vr] HUD texture dumped (vr_hud.tga, "
                                    "%dx%d)\n", hw, hh);
                            fflush(stderr);
                        }
                    }
...
/* The only caller of the eye pass; without ENABLE_VR the whole body is gone,
 * so no unused stub is left behind for the flat build to trip over. */
/* ---- VR settings persistence -------------------------------------------
 * One small key=value file beside the saves. The menu writes it on close;
 * startup reads it before the env knobs, so a pinned launch still wins. */
#ifdef ENABLE_VR
static int vr_settings_path(char *out, size_t cap) {
    char dir[512];
    int n;
    if (!mdkr_user_save_directory(dir, sizeof(dir))) {
        return 0;
    }
    n = snprintf(out, cap, "%s/vr_settings.cfg", dir);
    return n > 0 && (size_t)n < cap;
}
#endif

void mdkr_vr_settings_save(void) {
#ifdef ENABLE_VR
    char path[560];
    FILE *f;
    extern int mdkr_vr_menu_opacity(void);
    if (!vr_is_active()) {
        return;
    }
    if (!vr_settings_path(path, sizeof(path))) {
        return;
    }
    f = mdkr_fopen_utf8(path, "w");
    if (f == NULL) {
        fprintf(stderr, "[vr] settings save FAILED (%s)\n", path);
        return;
    }
    fprintf(f, "view_mode %d\n", vr_get_view_mode());
    fprintf(f, "world_units_per_m %.6g\n", (double)vr_units_per_meter());
    fprintf(f, "stereo %.6g\n", (double)vr_get_stereo_strength());
    fprintf(f, "fp_eyeheight %.6g\n", (double)vr_fp_eyeheight_units());
    fprintf(f, "fp_forward %.6g\n", (double)vr_fp_forward_units());
    fprintf(f, "tp_height %.6g\n", (double)vr_tp_eyeheight_units());
    fprintf(f, "tp_forward %.6g\n", (double)vr_tp_forward_units());
    fprintf(f, "hud_scale %.6g\n", (double)vr_get_hud_scale());
    fprintf(f, "hud_dist %.6g\n", (double)vr_get_hud_dist());
    fprintf(f, "horizon %d\n", vr_get_horizon_lock() ? 1 : 0);
    fprintf(f, "diorama_scale %.6g\n", (double)vr_get_diorama_scale());
    fprintf(f, "diorama_dist %.6g\n", (double)vr_get_diorama_dist());
    fprintf(f, "diorama_height %.6g\n", (double)vr_get_diorama_height());
    fprintf(f, "menu_opacity %d\n", mdkr_vr_menu_opacity());
    fclose(f);
    fprintf(stderr, "[vr] settings saved (%s)\n", path);
#endif
}

void mdkr_vr_settings_load(void) {
#ifdef ENABLE_VR
    char path[560];
    char key[64];
    double v;
    FILE *f;
    extern void mdkr_vr_menu_set_opacity(int pct);
    if (!vr_settings_path(path, sizeof(path))) {
        return;
    }
    f = mdkr_fopen_utf8(path, "r");
    if (f == NULL) {
        return;                     /* first run: nothing remembered yet */
    }
    /* The first-person seat pair as the file holds it, for the migration
     * below: a changed default is invisible to anyone who has ever saved,
     * so a file still carrying an OLD default pair is moved to the new one
     * (and only then -- any hand-set value in the pair stays put). */
    double fpHeightFile = 0.0, fpForwardFile = 0.0;
    int fpHeightSeen = 0, fpForwardSeen = 0;
    while (fscanf(f, "%63s %lf", key, &v) == 2) {
        if (strcmp(key, "view_mode") == 0) {
            vr_set_view_mode((int)v);
        } else if (strcmp(key, "world_units_per_m") == 0) {
            vr_set_world_scale((float)v);
        } else if (strcmp(key, "stereo") == 0) {
            vr_set_stereo_strength((float)v);
        } else if (strcmp(key, "fp_eyeheight") == 0) {
            vr_set_fp_eyeheight((float)v);
            fpHeightFile = v;
            fpHeightSeen = 1;
        } else if (strcmp(key, "fp_forward") == 0) {
            vr_set_fp_forward((float)v);
            fpForwardFile = v;
            fpForwardSeen = 1;
        } else if (strcmp(key, "hud_scale") == 0) {
            vr_set_hud_scale((float)v);
        } else if (strcmp(key, "hud_dist") == 0) {
            vr_set_hud_dist((float)v);
        } else if (strcmp(key, "horizon") == 0) {
            vr_set_horizon_lock(v != 0.0);
        } else if (strcmp(key, "diorama_scale") == 0) {
            vr_set_diorama_scale((float)v);
        } else if (strcmp(key, "diorama_dist") == 0) {
            vr_set_diorama_dist((float)v);
        } else if (strcmp(key, "diorama_height") == 0) {
            vr_set_diorama_height((float)v);
        } else if (strcmp(key, "menu_opacity") == 0) {
            mdkr_vr_menu_set_opacity((int)v);
        } else if (strcmp(key, "tp_height") == 0) {
            vr_set_tp_eyeheight((float)v);
        } else if (strcmp(key, "tp_forward") == 0) {
            vr_set_tp_forward((float)v);
        }
    }
    fclose(f);
    fprintf(stderr, "[vr] settings loaded (%s)\n", path);
    /* THE SHIPPED SEAT MOVES; A TUNED ONE DOES NOT.
     *
     * A saved config pins its values forever, so every time the default seat
     * changes the old default has to be recognised and carried forward, or a
     * player who never touched the row keeps a seat nobody ships any more.
     * Height 30 / slide back 12 is the current pair, set in the headset on the
     * Quest 3 build (2026-08-26). Before it: height 26 with the slide already
     * at -12, and before that every set had the slide at +16 with heights 128,
     * 70, 48 and 26. A file holding EXACTLY one of those pairs is a player who
     * never touched the seat and gets the new one. Any other pair is a choice
     * and is left alone. */
    if (fpHeightSeen && fpForwardSeen) {
        int wasShippedDefault = 0;
        if (fpForwardFile == 16.0) {
            const double oldHeights[] = { 128.0, 70.0, 48.0, 26.0 };
            size_t i;
            for (i = 0; i < sizeof(oldHeights) / sizeof(oldHeights[0]); i++) {
                if (fpHeightFile == oldHeights[i]) { wasShippedDefault = 1; break; }
            }
        } else if (fpForwardFile == -12.0 && fpHeightFile == 26.0) {
            wasShippedDefault = 1;
        }
        if (wasShippedDefault) {
            vr_set_fp_eyeheight(30.0f);
            vr_set_fp_forward(-12.0f);
            fprintf(stderr,
                    "[vr] first-person seat updated to the current default "
                    "(height 30, slide back 12; the file held the old default "
                    "%g / fwd %g)\n", fpHeightFile, fpForwardFile);
        }
    }
#endif
}
...
/*
 * THE VIEW KNOBS, ONCE PER RUN, for whichever path gets there first.
 *
 * These used to be set inside the eye pass, which only runs with a live
 * headset -- so the flat view modes started with a seat height of ZERO and
 * put the first-person camera at the kart's own origin, buried in the
 * scenery. The knobs describe the VIEW, not the session, so they belong to
 * both paths.
 *
 * Order is the contract, unchanged: DEFAULTS, then the REMEMBERED settings
 * file, then the env pins -- a pinned launch still wins, and an unpinned one
 * gets back exactly what was tuned last session.
 */
/*
 * THE DRAW DISTANCE ANSWERS FOR ITSELF.
 *
 * Under VR this port floors Enhancements.DrawDistance at 400 percent, which
 * is what stopped scenery popping in at the edge of vision. But a headset
 * has a deadline, and the captured log measured 12 to 21 percent of frames
 * being REPROJECTED -- invented by the compositor because the app missed it.
 * One frame in five synthesised is worse to look at than any pop-in: it is
 * the judder people describe as flickering.
 *
 * So the floor is no longer a constant. Every ten-second pacing window the
 * layer publishes what it measured, and this steps the floor DOWN a notch
 * while frames are being missed and back UP while they are not. Down fast,
 * up slow, and never below the authored 100: the enhancement gives ground
 * to the frame rate rather than the other way round, and a player who pinned
 * the value themselves is never overridden (the floor only ever raises a
 * value the config left lower, and a pin sits above it).
 *
 * Nothing here touches simulation. Draw distance is a draw-side enhancement
 * by construction (tracks.c), so a governor moving it cannot move a race.
 */
void mdkr_vr_quality_governor(void) {
#ifdef ENABLE_VR
    static const float kLadder[] = { 100.0f, 150.0f, 200.0f, 300.0f, 400.0f };
    static int rung = (int)(sizeof(kLadder) / sizeof(kLadder[0])) - 1;
    static unsigned seenSerial = 0u;
    static int calmWindows = 0;
    unsigned serial;
    float pct;
    ...
    serial = vr_reprojection_serial();
    if (serial == 0u || serial == seenSerial) {
        return;                 /* no fresh measurement to act on */
    }
    seenSerial = serial;
    pct = vr_reprojection_pct();
    ...
    if (pct > 8.0f) {
        ...
        if (rung > 0) {
            rung--;
            fprintf(stderr,
                    "[vr] %.0f%% of frames reprojected -- draw distance down "
                    "to %.0f%% to give the headset its deadline back\n",
                    (double)pct, (double)kLadder[rung]);
            /* _set_live, not _floor: a floor can only ever raise, and this
             * has to be able to give ground. Still live-only, still yields
             * to an env or CLI pin, still never persisted. */
            mdkr_video_config_vr_set_live(MDKR_ENH_DRAW_DISTANCE,
                                          kLadder[rung]);
        }
        return;
    }
    if (pct < 2.0f) {
        calmWindows++;
        /* Two calm windows before spending the headroom again, so a single
         * quiet stretch cannot start an oscillation. */
        if (calmWindows >= 2 &&
            rung < (int)(sizeof(kLadder) / sizeof(kLadder[0])) - 1) {
            calmWindows = 0;
            rung++;
            fprintf(stderr,
                    "[vr] frames steady (%.0f%% reprojected) -- draw distance "
                    "back up to %.0f%%\n",
                    (double)pct, (double)kLadder[rung]);
            /* _set_live, not _floor: a floor can only ever raise, and this
             * has to be able to give ground. Still live-only, still yields
...
void mdkr_vr_view_knobs_first_use(void) {
#ifdef ENABLE_VR
    static int done = 0;
    const char *eh, *fw, *th, *tf, *vm;
    if (done) {
        return;
    }
    done = 1;
    eh = getenv("MDKR_VR_FP_EYEHEIGHT");
    fw = getenv("MDKR_VR_FP_FORWARD");
    th = getenv("MDKR_VR_TP_HEIGHT");
    tf = getenv("MDKR_VR_TP_FORWARD");
    vm = getenv("MDKR_VR_VIEWMODE");
    /* First-person seat defaults are RAY'S TUNED VALUES (2026-08-16
     * in-headset round): height 70, slide forward 16. */
    vr_set_fp_eyeheight(70.0f);
    vr_set_fp_forward(16.0f);
    mdkr_vr_settings_load();
    if (eh != NULL && eh[0] != '\0') { vr_set_fp_eyeheight((float)atof(eh)); }
    if (fw != NULL && fw[0] != '\0') { vr_set_fp_forward((float)atof(fw)); }
    if (th != NULL && th[0] != '\0') { vr_set_tp_eyeheight((float)atof(th)); }
    if (tf != NULL && tf[0] != '\0') { vr_set_tp_forward((float)atof(tf)); }
    if (vm != NULL && vm[0] != '\0') { vr_set_view_mode(atoi(vm)); }
#endif
}

static void platform_vr_render_eyes_impl(uint64_t alphaNum, uint64_t alphaDen,
                                         int interpolated) {
#ifdef ENABLE_VR
    int w = 0, h = 0;
    if (!s_glReady) {
        return;                 /* GL path only: the session is bound to WGL */
    }
    /* First person: hand the layer the chase camera's and the racer head's
     * poses (donor SRB2 frame, converted in camera.c -- one home). Fed
     * before the XR frame opens so vr_update_eye_matrices composes against
     * this frame's poses. No feed (menus, cutscenes) locks first person so
     * the compose falls back to the plain head view instead of a stale
     * cockpit. */
    {
        extern int mdkr_vr_first_person_poses_at(
            uint64_t numerator, uint64_t denominator,
            float cam_pos[3], float cam_ypr[3],
            float head_pos[3], float head_ypr[3],
            float *cam_basis_yaw);
        float camPos[3], camYpr[3], headPos[3], headYpr[3];
        float camBasisYaw = 0.0f;
        static int fpDefaultsSet = 0;
        if (!fpDefaultsSet) {
            /* The knobs themselves are shared with the flat view modes, so
             * they live in one place now (above); whichever path arrives
             * first sets them. */
            mdkr_vr_view_knobs_first_use();
            /* Re-assert the LOD/draw-distance floors and the presentation
             * pace here too: the launcher boot path re-resolves the video
             * config after the startup decide, which would quietly undo an
             * early assert. Idempotent. */
            mdkr_video_config_vr_floor(MDKR_ENH_LOD_BIAS, 2.0f);
            mdkr_video_config_vr_floor(MDKR_ENH_DRAW_DISTANCE, 400.0f);
            mdkr_video_config_vr_set_live_text(MDKR_VIDEO_FRAME_LIMIT,
                                               "display");
            mdkr_video_config_vr_set_live_text(MDKR_VIDEO_MOTION_SMOOTHING,
                                               "interpolate");
            /* First eye render = the session is really up: tell the flat
             * window which view the headset woke in. */
            {
                extern void platformOverlayVrModeToast(const char *name);
                extern const char *vr_view_mode_label(void);
                platformOverlayVrModeToast(vr_view_mode_label());
            }
            fpDefaultsSet = 1;
        }
        /* At the frame's own alpha: the seat rides the same clock as the
         * world instead of stepping at tick rate under a smooth head --
         * the first-person jank. Alpha 0 on tick frames = the list's own
         * instant, consistent with the eye camera phase. */
        extern int mdkr_vr_player_race_finished(void);
        /* THE FINISH FLY-AROUND IS NOT A FIRST-PERSON SHOT. When the race is
         * won the game hands the postrace/cinematic camera a wide view of
         * your kart, and vr_set_action_cam un-hides the kart for it. If the
         * seat compose keeps running, that un-hidden kart sits at the near
         * plane filling both eyes -- "stereo stuck to face". Lock first
         * person down for the finish so the eyes ride the authored
         * fly-around camera like third person does. */
        if (mdkr_vr_player_race_finished()) {
            vr_set_fp_switch_locked(true);
            gfx_dkr_set_eye_hide_owner(NULL);
            vr_set_action_cam(0, 1);
        } else
        if (mdkr_vr_first_person_poses_at(alphaNum, alphaDen,
                                          camPos, camYpr, headPos, headYpr,
                                          &camBasisYaw)) {
            const float eyeheight = vr_fp_eyeheight_units();
            const float seatfwd = vr_fp_forward_units();
            extern const void *mdkr_vr_player_racer_transform(void);
            extern int mdkr_vr_player_race_finished(void);
            headPos[0] += seatfwd * cosf(headYpr[0]);
            headPos[1] += seatfwd * sinf(headYpr[0]);
            headPos[2] += eyeheight;
            /* Seat ORIENTATION comes from the CHASE CAMERA's basis (the
             * yaw the game itself eases and that was approved in third
             * person, look-behind flip included) with a LEVEL seat --
             * first person steers exactly like third person feels, from
             * inside the kart. The kart's raw heading and every bump used
             * to arrive 1:1 at the head, which was the residual clunk. */
            vr_set_cockpit_pose(headPos, camBasisYaw, 0.0f, 0.0f);
            gfx_dkr_set_eye_hide_owner(mdkr_vr_player_racer_transform());
            /* The finish-line fly-around films YOUR OWN kart driving off:
             * an invisible star of the show is a broken ending, so the
             * first-person hide stands down the moment the race is won. */
            vr_set_action_cam(0, mdkr_vr_player_race_finished());
            /* MDKR_VR_EYEDUMP_AT_FINISH=<burst>: arm the eye dump (and the
             * HUD texture dump) when the player's race finishes, so the
             * results flow can be dumped headless without guessing a frame
             * number. Fires once per run, 30 stereo frames after the finish
             * (the board is up by then), plus a second burst 240 later. */
            {
                static int finishDump = -2;
                static int finishArmed = 0;
                if (finishDump == -2) {
                    const char *e = getenv("MDKR_VR_EYEDUMP_AT_FINISH");
                    finishDump = (e != NULL && e[0] != '\0') ? atoi(e) : -1;
                    if (finishDump == 0) finishDump = 1;
                }
                if (finishDump > 0 && finishArmed < 2 &&
                    mdkr_vr_player_race_finished()) {
                    static Uint32 lastArmAt = 0;
                    const Uint32 now = SDL_GetTicks();
                    if (finishArmed == 0 || now - lastArmAt > 4000u) {
                        vr_arm_eye_dump(finishArmed == 0 ? 30 : 240,
                                        finishDump);
                        s_vrHudDumpAtFinish = 1;
                        finishArmed++;
                        lastArmAt = now;
                    }
                }
            }
            /* BEFORE vr_set_game_view: that call ends by recomposing the
             * mode views, so a basis fed after it would be a frame late.
             * BOTH yaws carry the look-behind flip: the base eye view is
             * built flipped, and feeding the plain yaw here made the
             * first-person compose cancel the flip back out -- a rear
             * glance that showed nothing but a wobble. With the flip in
             * both, their difference (the kart-vs-camera heading) is
             * unchanged and the flip itself reaches the eyes as a cut. */
            vr_set_game_view_basis_yaw(camBasisYaw);
            vr_set_game_view(camPos, camBasisYaw, camYpr[1], camYpr[2], false);
            vr_set_fp_switch_locked(false);
            /* MDKR_VR_TRACE_FP=1: one line per present with the whole
             * first-person feed, so a "turning feels jerky" report can be
             * read as a SERIES (steps, duplicates, fallbacks) instead of
             * argued from the matrix. */
            {
                static int traceFp = -1;
                if (traceFp < 0) {
                    const char *e = getenv("MDKR_VR_TRACE_FP");
                    traceFp = (e != NULL && e[0] != '\0' && e[0] != '0') ? 1 : 0;
                }
                if (traceFp) {
                    extern int mdkr_vr_fp_resolve_flags(void);
                    fprintf(stderr,
                            "[vrfp] a=%llu/%llu res=%d cam=%.3f,%.3f,%.3f "
                            "camYaw=%.5f basis=%.5f head=%.3f,%.3f,%.3f "
                            "headYaw=%.5f seat=%.5f t=%.4f\n",
                            (unsigned long long)alphaNum,
                            (unsigned long long)alphaDen,
                            mdkr_vr_fp_resolve_flags(),
                            (double)camPos[0], (double)camPos[1],
                            (double)camPos[2], (double)camYpr[0],
                            (double)camBasisYaw, (double)headPos[0],
                            (double)headPos[1], (double)headPos[2],
                            (double)headYpr[0], (double)vr_get_cockpit_yaw(),
    ...
    /* Open the XR frame here rather than at the swap. This is also where the
     * donor said it has to end up once eyes render, so xrWaitFrame paces the
     * render instead of the swap tail. */
    vr_begin_frame();
    /* Size the eye render from the headset, every frame (see the function:
     * it both raises and lowers, so a window resize can never leave a
     * multiplier behind that the driver has to refuse). */
    vr_apply_eye_render_scale();
    platform_sdl_drawable_size(&w, &h);
    /* The display list rasters at the RENDER resolution (output x
     * Video.RenderScale) and its viewport/scissor commands carry those
     * coordinates. Sizing the eye target from the window drawable instead
     * drew a scale-2 frame into a half-size target: the lower quarter of the
     * scene at 2x magnification, measured against the panel dump as a
     * road-filling zoom that no separation change could move. */
    if (gfx_current_dimensions.width != 0u &&
        gfx_current_dimensions.height != 0u) {
        w = (int)gfx_current_dimensions.width;
        h = (int)gfx_current_dimensions.height;
    }
    vr_render_stereo_eyes(w, h, alphaNum, alphaDen, interpolated);
#else
    (void)alphaNum;
    (void)alphaDen;
    (void)interpolated;
...
void platform_vr_render_eyes(void) {
    platform_vr_render_eyes_impl(0u, 1u, 0);
}

void platform_vr_render_eyes_interpolated(uint64_t numerator,
                                          uint64_t denominator) {
    platform_vr_render_eyes_impl(numerator, denominator, 1);
}

/*
 * VR motion controllers, merged into port 0 (the donor layer's pad-0 merge).
 *
 * The OpenXR action set is created and attached by platform/vr/vr_openxr.cpp;
 * this is only the DKR-side translation from its device-independent state
 * (vr_controller_buttons/vr_controller_stick, already digitised with 60%/40%
 * press/release hysteresis) into the N64 pad the game reads. Merged, never
 * replacing: keyboard and a real gamepad keep working at the same time, so a
 * desk debug session and a headset session use the same build.
 *
 * The layout is FIXED (the house rule: a VR port ships a layout, not a
 * bindings editor) and reads as a racing pad (revised 2026-08-12 for the established
 * layout: triggers and grips drift, left trigger shoots, Y looks back):
 *
 *   A                  -> A       accelerate
 *   B                  -> B       brake and reverse
 *   left trigger  / X  -> Z_TRIG  fire the item you are holding
 *   right trigger      -> R_TRIG  hop, and hold it to power-slide
 *   both grips         -> R_TRIG  same power-slide, for grip-drifters
 *   Y (hold)           -> look behind (port feature; the game has no rear view)
 *   left stick         -> analog stick, full N64 range (steer, and menus)
 *   right stick X/Y    -> C buttons (camera zoom / course directions)
 *   left stick CLICK   -> START (tap alone; menu button also works where the
 *                         runtime does not claim it)
 *   right stick CLICK  -> cycle VR view mode (third person <-> diorama)
 *   both stick clicks  -> hold ~0.5 s to recenter
 *
 * START also answers to both face buttons pressed together, because the
 * runtime owns the physical menu button on some devices (Virtual Desktop
 * opens its own dash on it) and a press the runtime eats can never reach us.
 *
 * Ranges are taken from this port's own constants rather than assumed: the
 * stick is +/-STICK_FULL (80), which is what the game's accessors normalise
 * against. A stick fed at the wrong scale reads as a dead or weak control and
 * looks exactly like a tuning problem.
 */
#ifdef ENABLE_VR
static unsigned int vr_pad_buttons_overlay(void) {
    unsigned int mask = 0u;
    unsigned int vr;
    int fpL3Glance = 0;

    if (!vr_controllers_active()) {
        s_lookBehindVr = 0;     /* never leave a held flag behind */
        return 0u;
    }
    vr = vr_controller_buttons();

    /*
     * Thumbstick clicks (revised 2026-08-12 for the established layout):
     *
     *   LEFT stick click, tapped alone  -> START (pause). The menu button
     *     stays START too, but Virtual Desktop claims it on some devices, so
     *     a single unclaimed button must exist.
     *   RIGHT stick click, tapped alone -> cycle the VR VIEW MODE: third
     *     person -> first person -> diorama. Theater is deliberately NOT in
     *     the cycle (it is the menu fallback, and landing in it from a
     *     stick click reads as the VR breaking).
     *   BOTH sticks held ~0.5 s        -> recenter (the VR layer owns it).
     *
     * Both actions fire on RELEASE, and only when the other stick was never
     * down during the press -- so starting a recenter can never pause the
     * game or flip the view mode on the way in.
     */
    {
        static unsigned sPrevSticks;
        static int sLeftContaminated, sRightContaminated;
        static Uint32 sStartPulseUntil;
        static Uint32 sLeftDownAt;
        const unsigned lNow = vr & VR_BTN_LSTICK;
        const unsigned rNow = vr & VR_BTN_RSTICK;
        const unsigned lPrev = sPrevSticks & VR_BTN_LSTICK;
        const unsigned rPrev = sPrevSticks & VR_BTN_RSTICK;
        /* In FIRST PERSON the left click doubles as look-behind: a HOLD past
         * the tap window is a rear glance (L3 should look
         * behind in first person). The threshold is what keeps the two
         * meanings apart -- a tap under it still pauses, a hold over it
         * glances and its release no longer pauses. Y stays the instant
         * glance in every mode. */
        const Uint32 kTapMs = 250u;

        if (lNow && !lPrev) {
            sLeftDownAt = SDL_GetTicks();
        }
        if (lNow && rNow) {
            sLeftContaminated = 1;
            sRightContaminated = 1;
        }
        if (lNow && !sLeftContaminated &&
            vr_get_view_mode() == VR_VIEW_FIRST_PERSON &&
            SDL_GetTicks() >= sLeftDownAt + kTapMs) {
            fpL3Glance = 1;
        }
        if (!lNow && lPrev) {
            if (!sLeftContaminated &&
                !(vr_get_view_mode() == VR_VIEW_FIRST_PERSON &&
                  SDL_GetTicks() >= sLeftDownAt + kTapMs)) {
                /* A ~120 ms level, not a one-call pulse: the pad can be read
                 * several times per tick and a single-call pulse can fall
                 * between the game's samples. */
                sStartPulseUntil = SDL_GetTicks() + 120u;
            }
            sLeftContaminated = 0;
        }
        if (!rNow && rPrev) {
            if (!sRightContaminated) {
                /* The donor cycle: third person -> first person -> diorama,
                 * theater deliberately excluded (menu-only; landing on a
                 * flat screen from a stick click reads as the VR breaking).
                 * The cycle joined first person when the cockpit feed
                 * landed. The layer logs the mode by name. */
                vr_cycle_view_mode();
            }
            sRightContaminated = 0;
        }
        sPrevSticks = vr & (VR_BTN_LSTICK | VR_BTN_RSTICK);

        if (vr & VR_BTN_MENU) {
            mask |= N64_START;
        }
        if (SDL_GetTicks() < sStartPulseUntil) {
            mask |= N64_START;
        }
    }

    /* Say what the runtime actually delivers, once per change. A button the
     * runtime swallows is invisible otherwise, and looks exactly like a wrong
     * mapping. */
    {
        static unsigned lastVr = 0u;
        static int changesLogged = 0;
        if (vr != lastVr && changesLogged < 24) {
            lastVr = vr;
            changesLogged++;
            fprintf(stderr, "[vr] controller buttons: 0x%04x%s\n", vr,
                    (vr & VR_BTN_MENU) ? " (MENU arrived)" : "");
            fflush(stderr);
        }
    }

    if (vr & VR_BTN_A)                     mask |= N64_A;   /* accelerate */
    if (vr & VR_BTN_B)                     mask |= N64_B;   /* brake / reverse */
    if (vr & (VR_BTN_LTRIGGER | VR_BTN_X)) mask |= N64_Z;   /* fire item */
    if (vr & (VR_BTN_RTRIGGER | VR_BTN_RGRIP | VR_BTN_LGRIP))
                                           mask |= N64_R;   /* hop / power-slide */
    /* Y is look-behind, not an N64 button; held state, view builder reads it.
     * The first-person L3 hold joins it (decided in the stick block above). */
    s_lookBehindVr = ((vr & VR_BTN_Y) || fpL3Glance) ? 1 : 0;

    /* Right stick -> C buttons, past a deliberate deadzone so a resting thumb
     * never nudges the camera. */
    {
        float rs[2];
        vr_controller_stick(1, rs);
        if (rs[0] < -0.5f) mask |= N64_CL;
        if (rs[0] >  0.5f) mask |= N64_CR;
        if (rs[1] >  0.5f) mask |= N64_CU;
        if (rs[1] < -0.5f) mask |= N64_CD;
    }
    return mask;
}

/* Left stick -> N64 analog, in the game's own +/-STICK_FULL units. Returns
 * false when VR is not feeding, so the caller keeps the desk pad untouched. */
static int vr_pad_stick_overlay(int *sx, int *sy) {
    float ls[2];
    int x, y;
    ...
    x = (int)(ls[0] * (float)STICK_FULL);
    y = (int)(ls[1] * (float)STICK_FULL);   /* OpenXR +y is up, same as the N64 stick */
    if (x > STICK_FULL) x = STICK_FULL;
    if (x < -STICK_FULL) x = -STICK_FULL;
    if (y > STICK_FULL) y = STICK_FULL;
    if (y < -STICK_FULL) y = -STICK_FULL;
    if (x == 0 && y == 0) {
        return 0;   /* nothing to say: let the desk pad through unchanged */
    }
    if (sx) *sx = x;
    if (sy) *sy = y;
    return 1;
...
#ifdef ENABLE_VR
    if (port == 0) {
        return s_pads[0].buttons | vr_pad_buttons_overlay();
    }
#endif
...
#ifdef ENABLE_VR
    /* Deflected VR stick wins over a centred desk stick; a centred VR stick
     * never overrides a real one. */
    if (port == 0) {
        (void)vr_pad_stick_overlay(sx, sy);
    }
#endif
}
